//! BDD: end-to-end verification that the broker injects credentials on
//! decrypted HTTPS requests inside the MITM tunnel. The upstream is a
//! tiny rustls server that echoes the Authorization header it received
...
pub struct IWorld {
    ...
    /// Authorization header that the upstream observed (set by the stub).
...
#[given(expr = "the broker holds a secret {string} with value {string}")]
fn stash_secret(world: &mut IWorld, key: String, value: String) {
    let s = Arc::new(InMemorySecretStore::new());
    s.set(key, value);
    world.secrets = Some(s);
}

#[given("a policy that allows the upstream on CONNECT and bearer-injects \"@profile.openai_key\" on GET")]
fn build_broker(world: &mut IWorld) {
    install_provider();
...
fn run_client(world: &mut IWorld, sandbox_auth: String) {
    ...
    rt.block_on(async move {
        ...
        // TLS handshake with broker (trust the broker CA only).
        let mut roots = RootCertStore::empty();
...
fn main() {
    futures::executor::block_on(IWorld::run("tests/features/broker_inject.feature"));
}
