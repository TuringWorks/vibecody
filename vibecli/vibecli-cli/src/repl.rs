...
fn command_hint(cmd: &str) -> Option<&'static str> {
    match cmd {
        ...
        "/superbrain" => Some("[consensus|chain|explain|routes] <prompt>  — route by task category to the configured provider"),
        ...
        "/recipe"  => Some("[list | show <file>]  — list or inspect bundled/local recipes"),
        "/rewind"   => Some("[list | <timestamp>]  — save or restore a conversation checkpoint"),
        "/workspace-detect" => Some("— detect workspace languages/frameworks and suggest skills"),
        ...
        "/git-init" => Some("[--github]  — create a git repository here, optionally on GitHub too"),
