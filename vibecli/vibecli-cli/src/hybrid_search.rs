    ...
    pub fn score(&self, bm25: f64, vector: f64, bm25_rank: usize, vec_rank: usize) -> f64 {
        match self {
            ...
            Self::Linear {
                bm25_weight,
                vector_weight,
            } => bm25_weight * bm25 + vector_weight * vector,
            Self::Rrf { k } => 1.0 / (k + bm25_rank as f64) + 1.0 / (k + vec_rank as f64),
        }
    }
}
    ...
    pub fn score(&self, doc: &SearchDoc, query_terms: &[String]) -> f64 {
        ...
        for term in query_terms {
            ...
            let tf_norm = tf * (self.k1 + 1.0)
                / (tf
                    + self.k1
                        * (1.0 - self.b + self.b * doc.doc_len as f64 / self.avg_doc_len.max(1.0)));
            score += idf * tf_norm;
        }
        score
    }
...
impl HybridSearch {
    ...
    pub fn search(
        ...
        // Rerank
        let bm25_rank_map: HashMap<usize, usize> = bm25_hits
            .iter()
            .enumerate()
            .map(|(r, (i, _))| (*i, r + 1))
            .collect();
        let vec_rank_map: HashMap<usize, usize> = vec_scores
            .iter()
            .enumerate()
            .map(|(r, (i, _))| (*i, r + 1))
            .collect();
        let bm25_score_map: HashMap<usize, f64> = bm25_hits.iter().map(|(i, s)| (*i, *s)).collect();
        let vec_score_map: HashMap<usize, f64> =
            vec_scores.iter().map(|(i, s)| (*i, *s as f64)).collect();
    ...
    pub fn multi_hop(&self, query_a: &str, query_b: &str, limit: usize) -> Vec<SearchResult> {
        let hits_a: std::collections::HashSet<String> = self
            .bm25
            .search(query_a, 50)
            .iter()
            .map(|(i, _)| self.bm25.docs()[*i].id.clone())
            .collect();
        self.search(query_b, None, limit * 2)
            .into_iter()
...
pub fn extract_snippet(content: &str, query: &str, context_lines: usize) -> String {
    ...
    let match_line = lines
        .iter()
        .position(|l| l.to_lowercase().contains(first_term));
...
mod tests {
    ...
    fn test_bm25_search_finds_relevant() {
        ...
        idx.add(make_doc(
            ...
            "a.rs",
            "rust",
            "fn authenticate_user(username password) {}",
        ));
        idx.add(make_doc(
            "d2",
            "b.rs",
            "rust",
            "fn render_template(html) {}",
        ));
    ...
    fn test_hybrid_search_bm25_only() {
        ...
        hs.add_doc(make_doc(
            ...
            "auth.rs",
            "rust",
            "fn authenticate_user(token) {}",
        ));
        hs.add_doc(make_doc(
            "d2",
            "render.rs",
            "rust",
            "fn render_html(template) {}",
        ));
    ...
    fn test_hybrid_search_with_vector() {
        let mut hs = HybridSearch::new(RerankStrategy::Linear {
            bm25_weight: 0.5,
            vector_weight: 0.5,
        });
        hs.add_doc(make_doc_with_emb(
            "d1",
            "auth token validate",
            vec![1.0, 0.0, 0.0],
        ));
    ...
    fn test_hybrid_multi_hop_intersects() {
        ...
        hs.add_doc(make_doc(
            ...
            "a.rs",
            "rust",
            "auth token validate session import db",
        ));
        hs.add_doc(make_doc(
            "d2",
            "b.rs",
            "rust",
            "auth token without database",
        ));
        hs.add_doc(make_doc(
            "d3",
            "c.rs",
            "rust",
            "render template html without auth",
        ));
