...
impl CounselSession {
    ...
    pub fn build_messages(&self, participant_index: usize, round_number: usize) -> Vec<Message> {
        ...
        if round_number > 0 && !self.rounds.is_empty() {
            ...
            for round in &self.rounds {
                ...
                if let Some(ref interjection) = round.user_interjection {
                    context.push_str(&format!("[User follow-up]: {}\n\n", interjection));
                }
            }
            context.push_str(&format!(
    ...
    /// Vote on a response.
    pub fn vote(&mut self, round_index: usize, participant_index: usize, delta: i32) {
...
mod tests {
    ...
    fn test_session_creation() {
        let session = CounselSession::new("Should we use Rust?".into(), make_participants(), 0);
    ...
    fn test_round1_messages_topic_only() {
        let session = CounselSession::new("Is Rust good?".into(), make_participants(), 0);
    ...
    fn test_round2_includes_previous() {
        let mut session = CounselSession::new("Test topic".into(), make_participants(), 0);
    ...
    fn test_user_interjection() {
        let mut session = CounselSession::new("Topic".into(), make_participants(), 0);
    ...
    fn test_synthesis_prompt() {
        let mut session = CounselSession::new("Test".into(), make_participants(), 0);
        session.add_round(vec![
            CounselResponse {
                participant_index: 0,
    ...
    fn test_current_round() {
        let mut session = CounselSession::new("T".into(), make_participants(), 0);
