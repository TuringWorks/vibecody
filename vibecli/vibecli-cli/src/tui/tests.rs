...
// These previously restated the fallback literal inline and asserted that
// `Option::unwrap_or_else` works — so they kept passing while the real
// defaults moved on (this file still claimed `gpt-4-turbo` and
// `claude-3-opus-20240229`). They now read the same table the provider
// constructor reads, which is the only thing that makes them able to fail.
#[test]
fn default_model_is_defined_for_every_provider_the_tui_constructs() {
    for provider in ["ollama", "openai", "anthropic", "claude", "gemini", "grok"] {
        assert!(
            crate::tui::default_model_for(provider).is_some(),
            "{provider} is constructible but has no default model"
        );
    }
}

#[test]
fn default_model_is_absent_for_an_unknown_provider() {
    // Absent, not a plausible-looking guess: an unknown name must surface
    // as an error, never as a silent fallback to some other vendor's model.
    assert_eq!(crate::tui::default_model_for("not-a-provider"), None);
}

#[test]
fn claude_and_anthropic_name_the_same_default() {
    assert_eq!(
        crate::tui::default_model_for("claude"),
        crate::tui::default_model_for("anthropic")
    );
}
