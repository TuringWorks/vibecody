    ...
    pub fn bounds(&self) -> BoundingBox {
        match self {
            ...
            Self::Circle { cx, cy, radius, .. } => {
                BoundingBox::new(cx - radius, cy - radius, radius * 2.0, radius * 2.0)
            }
    ...
    fn generate_react(
        ...
        for shape in recognized {
            let comp = match &shape.recognized_as {
                UiComponent::Button => {
                    "      <button className=\"btn\">Click me</button>".to_string()
                }
                UiComponent::Input => {
                    "      <input className=\"input\" placeholder=\"Type here...\" />".to_string()
                }
                UiComponent::Card => {
                    "      <div className=\"card\"><p>Card content</p></div>".to_string()
                }
                UiComponent::Navbar => {
                    "      <nav className=\"navbar\"><span>Logo</span></nav>".to_string()
                }
                UiComponent::Sidebar => {
                    "      <aside className=\"sidebar\">Sidebar</aside>".to_string()
                }
                UiComponent::Image => {
                    "      <img src=\"placeholder.png\" alt=\"image\" />".to_string()
                }
                UiComponent::Divider => "      <hr />".to_string(),
                _ => format!(
                    "      <div className=\"{}\">{}</div>",
                    shape.recognized_as.name().to_lowercase(),
                    shape.recognized_as.name()
                ),
            };
...
impl SketchCanvas {
        ...
        for (i, elem) in self.elements.iter().enumerate() {
            ...
            let (component, confidence) = match elem {
                ...
                CanvasElement::Text { content, .. } => {
                    if content.to_lowercase().contains("button")
                        || content.to_lowercase().contains("submit")
                    {
                        (UiComponent::Button, 0.9)
                    } else if content.to_lowercase().contains("nav")
                        || content.to_lowercase().contains("menu")
                    {
                ...
                CanvasElement::Arrow { .. } => (UiComponent::Divider, 0.3),
    ...
    pub fn infer_layout(&mut self) -> LayoutInference {
        ...
        let x_spread: f64 = if sorted_x.len() > 1 {
            sorted_x.last().expect("non-empty").bounds.x
                - sorted_x.first().expect("non-empty").bounds.x
        } else {
            0.0
        };
...
mod tests {
    ...
    fn test_add_element() {
        let mut c = canvas();
        c.add_element(CanvasElement::Rectangle {
            x: 0.0,
            y: 0.0,
            width: 100.0,
            height: 40.0,
    ...
    fn test_clear() {
        let mut c = canvas();
        c.add_element(CanvasElement::Circle {
            cx: 50.0,
            cy: 50.0,
            radius: 25.0,
            fill: None,
    ...
    fn test_recognize_input() {
        ...
        assert!(c
            .recognized
            .iter()
            .any(|r| r.recognized_as == UiComponent::Input));
    }

    #[test]
    fn test_recognize_divider() {
        let mut c = canvas();
        c.add_element(CanvasElement::Line {
            x1: 0.0,
            y1: 100.0,
            x2: 500.0,
            y2: 100.0,
        ...
        assert!(c
            .recognized
            .iter()
            .any(|r| r.recognized_as == UiComponent::Divider));
    }

    #[test]
    fn test_recognize_text_button() {
        let mut c = canvas();
        c.add_element(CanvasElement::Text {
            x: 10.0,
            y: 10.0,
            content: "Submit Button".to_string(),
            font_size: 14.0,
        });
        c.recognize_shapes();
        assert!(c
            .recognized
            .iter()
            .any(|r| r.recognized_as == UiComponent::Button));
    }
    ...
    fn test_recognize_navbar() {
        ...
        assert!(c
            .recognized
            .iter()
            .any(|r| r.recognized_as == UiComponent::Navbar));
    }
    ...
    fn test_recognize_confidence_filter() {
        ...
        c.add_element(CanvasElement::Freehand {
    ...
    fn test_export_svg_arrow() {
        let mut c = canvas();
        c.add_element(CanvasElement::Arrow {
            x1: 0.0,
            y1: 0.0,
            x2: 100.0,
            y2: 50.0,
