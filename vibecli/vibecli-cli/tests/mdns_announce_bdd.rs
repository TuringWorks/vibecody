...
fn build_packet(world: &mut MdnsWorld) {
    let ips: Vec<std::net::Ipv4Addr> = world
        .lan_ip
        .as_ref()
...
fn call_get_hostname(world: &mut MdnsWorld) {
    world.hostname = std::env::var("HOSTNAME").unwrap_or_else(|_| {
    ...
    world.hostname = world
        .hostname
        .split('.')
        .next()
        .unwrap_or("vibecli-host")
        .to_string();
}
...
fn check_root_null(world: &mut MdnsWorld) {
    assert_eq!(
        *world.encoded.last().unwrap(),
        0,
        "last byte should be 0 (root label)"
    );
}
...
fn check_flags(world: &mut MdnsWorld, expected: u32) {
    ...
    assert_eq!(
        flags, expected,
        "flags mismatch: got {flags:#06x}, want {expected:#06x}"
    );
}
...
fn check_answer_count(world: &mut MdnsWorld, min: u16) {
    assert!(
        world.answer_count >= min,
        "answer count {}, expected >= {min}",
        world.answer_count
    );
}
...
fn check_no_dot(world: &mut MdnsWorld) {
    assert!(
        !world.hostname.contains('.'),
        "hostname '{}' should not contain '.'",
        world.hostname
    );
}
...
fn check_no_loopback(world: &mut MdnsWorld, loopback: String) {
    ...
    for ip in &world.local_ips {
        assert_ne!(
            *ip, lo,
            "loopback address should not appear in local_ipv4_addrs"
        );
    }
}

fn main() {
    futures::executor::block_on(MdnsWorld::run("tests/features/mdns_announce.feature"));
}
