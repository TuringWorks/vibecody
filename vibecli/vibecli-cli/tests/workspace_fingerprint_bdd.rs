...
fn single_path(world: &mut WfWorld, path: String) {
    world.path_a = path;
}

#[given(expr = "workspace paths {string} and {string}")]
fn two_paths(world: &mut WfWorld, a: String, b: String) {
    world.path_a = a;
    world.path_b = b;
}
...
fn compute_one(world: &mut WfWorld) {
...
fn check_identical(world: &mut WfWorld) {
    assert_eq!(world.fp_a, world.fp_b);
}

#[then("they should differ")]
fn check_differ(world: &mut WfWorld) {
    assert_ne!(world.fp_a, world.fp_b);
}
