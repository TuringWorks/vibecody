...
fn assert_in_focus(world: &mut FvWorld) {
    assert!(
        world.manager.is_in_focus(),
        "expected manager to be in focus"
    );
}
...
fn assert_not_in_focus(world: &mut FvWorld) {
    assert!(
        !world.manager.is_in_focus(),
        "expected manager to NOT be in focus"
    );
}
...
fn assert_auto_exit_true(world: &mut FvWorld, now: u64) {
    assert!(
        world.manager.should_auto_exit(now),
        "expected auto-exit at {}",
        now
    );
}
