...
**Scope:** 22 new gaps from FIT-GAP-ANALYSIS-v7.md; 9 implementation phases across 4 priority tiers

## Current State

All phases from Roadmap v1 (1-5), v2 (6-9), v3 (10-14), and v4 (15-22) are **complete**. FIT-GAP v6 (19 gaps) is **all closed**. FIT-GAP v7 identifies **22 new gaps** based on analysis of **35+ competitors**.
...
- [ ] `a2a_protocol.rs` — Full A2A implementation:
...
- [ ] `A2aPanel.tsx` — Browse discovered agents, send tasks, view task progress, agent card editor
- [ ] REPL: `/a2a card|serve|discover|call|tasks|status`
- [ ] Tests: 55+ unit tests
- [ ] Skill file: `skills/a2a-protocol.md`

### 23.2 Agent Skills Standard Compatibility

**Deliverables:**

- [ ] `agent_skills_compat.rs` — Cross-tool skills interop:
...
- [ ] Extend existing REPL: `/skills import|export|search|validate|publish`
- [ ] Tests: 35+ unit tests
...
- [ ] `worktree_pool.rs` — Lightweight parallel agent execution:
...
- [ ] `WorktreePoolPanel.tsx` — Grid view of active worktrees, per-agent progress, merge queue
- [ ] REPL: `/worktree spawn|list|merge|cleanup|config`
- [ ] Tests: 50+ unit tests
...
- [ ] `agent_host.rs` — Host external agents alongside VibeCody:
...
- [ ] `AgentHostPanel.tsx` — Multi-agent dashboard with per-agent output panes
- [ ] REPL: `/host add|list|route|remove|ask`
- [ ] Tests: 40+ unit tests
...
- [ ] `proactive_agent.rs` — Background intelligence scanner:
...
- [ ] `ProactivePanel.tsx` — Suggestion feed with accept/reject/snooze, category filters, learning stats
- [ ] REPL: `/proactive scan|config|accept|reject|history|digest`
- [ ] Tests: 45+ unit tests
...
- [ ] `issue_triage.rs` — Automated issue processing:
...
- [ ] `TriagePanel.tsx` — Issue queue with triage suggestions, bulk actions, triage rules editor
- [ ] REPL: `/triage run|rules|labels|history|batch`
- [ ] Tests: 40+ unit tests
...
- [ ] `web_grounding.rs` — Integrated web search for agent loop:
...
- [ ] `WebGroundingPanel.tsx` — Search history, cached results, provider config
- [ ] REPL: `/search web|cache|providers|config`
- [ ] Tests: 40+ unit tests
...
- [ ] `semantic_index.rs` — AST-level codebase understanding:
...
- [ ] `SemanticIndexPanel.tsx` — Visual call graph, type hierarchy tree, dependency explorer
- [ ] REPL: `/index build|query|callers|callees|hierarchy|deps|stats`
- [ ] Tests: 55+ unit tests
...
- [ ] `mcp_streamable.rs` — Next-gen MCP transport:
...
- [ ] Extend `McpPanel.tsx` with OAuth config, remote server management
- [ ] REPL: `/mcp serve-http|oauth|tokens|remote`
- [ ] Tests: 45+ unit tests
...
- [ ] `mcts_repair.rs` — Monte Carlo Tree Search for code repair:
...
- [ ] `MctsRepairPanel.tsx` — Tree visualization, exploration stats, cost comparison
- [ ] REPL: `/repair mcts|agentless|compare|config`
- [ ] Tests: 50+ unit tests
...
- [ ] `cost_router.rs` — Smart task-to-model routing:
...
- [ ] `CostRouterPanel.tsx` — Cost breakdown per model, routing decisions log, budget status
- [ ] REPL: `/route cost|budget|model|stats|compare`
- [ ] Tests: 40+ unit tests
...
- [ ] `visual_verify.rs` — Automated UI verification:
...
- [ ] `VisualVerifyPanel.tsx` — Side-by-side comparison, diff overlay, baseline management
- [ ] REPL: `/verify screenshot|diff|baseline|ci`
- [ ] Tests: 35+ unit tests
...
- [ ] `next_task.rs` — Workflow-level prediction:
...
- [ ] `NextTaskPanel.tsx` — Suggestion sidebar with accept/dismiss, prediction accuracy stats
- [ ] REPL: `/nexttask suggest|accept|reject|learn|stats`
- [ ] Tests: 40+ unit tests
...
- [ ] `voice_local.rs` — Local speech recognition:
...
- [ ] Extend voice REPL: `/voice local|model|download|config`
- [ ] Tests: 30+ unit tests
...
- [ ] `doc_sync.rs` — Bidirectional spec-code synchronization:
...
- [ ] `DocSyncPanel.tsx` — Sync status dashboard, drift visualization, reconciliation actions
- [ ] REPL: `/docsync status|reconcile|watch|freshness`
- [ ] Tests: 35+ unit tests
...
- [ ] `native_connectors.rs` — Pre-built service connectors:
...
- [ ] `ConnectorsPanel.tsx` — Connector grid with status, one-click setup, webhook logs
- [ ] REPL: `/connect list|add|test|remove|webhook`
- [ ] Tests: 50+ unit tests
...
- [ ] `agent_analytics.rs` — Admin-facing usage analytics:
...
- [ ] `AnalyticsPanel.tsx` — Charts, filters, export buttons, ROI dashboard
- [ ] REPL: `/analytics dashboard|export|roi|compare`
- [ ] Tests: 40+ unit tests
...
- [ ] `agent_trust.rs` — Trust and confidence system:
...
- [ ] `TrustPanel.tsx` — Trust scores per model, trend charts, threshold config
- [ ] REPL: `/trust scores|history|config|explain`
- [ ] Tests: 35+ unit tests
...
- [ ] `smart_deps.rs` — Intelligent dependency management:
...
- [ ] `SmartDepsPanel.tsx` — Dependency graph visualization, conflict resolution wizard, CVE dashboard
- [ ] REPL: `/deps resolve|compare|patch|audit|graph`
- [ ] Tests: 40+ unit tests
...
- [ ] `rlcef_loop.rs` — Execution-based learning:
...
- [ ] `RlcefPanel.tsx` — Learning curves, mistake clusters, strategy effectiveness charts
- [ ] REPL: `/rlcef train|eval|mistakes|patterns|reset|export`
- [ ] Tests: 45+ unit tests
...
- [ ] `langgraph_bridge.rs` — Python agent ecosystem compatibility:
...
- [ ] `LangGraphPanel.tsx` — Pipeline visualization, node config, checkpoint browser
- [ ] REPL: `/langgraph serve|connect|status|checkpoint`
- [ ] Tests: 35+ unit tests
...
- [ ] `sketch_canvas.rs` — Freeform drawing to code:
...
- [ ] `SketchCanvasPanel.tsx` — Drawing canvas, shape palette, generated code preview, export
- [ ] REPL: `/sketch new|recognize|generate|export`
- [ ] Tests: 30+ unit tests
...
| Phase | Priority | Est. Days | Cumulative |
|-------|----------|-----------|------------|
| 23: Dual-Protocol | P0 | 3-4 | 3-4 |
| 24: Parallel Workers | P0 | 5-6 | 8-10 |
| 25: Proactive Intelligence | P0 | 4-5 | 12-15 |
| 26: Grounding & Context | P0/P1 | 6-7 | 18-22 |
| 27: MCP Evolution | P1 | 3-4 | 21-26 |
| 28: Smart Repair | P1/P2 | 5-6 | 26-32 |
| 29: Developer Experience | P1/P2 | 6-7 | 32-39 |
| 30: Enterprise | P2/P3 | 6-7 | 38-46 |
| 31: Strategic Frontiers | P3 | 5-6 | 43-52 |

**Total estimated effort:** 43-52 development days
...
| Metric | Target |
|--------|--------|
| All 22 v7 gaps closed | Yes |
| New Rust modules | 22 |
| New unit tests | 910+ (total ~7,538+) |
| New VibeUI panels | 19 (total ~183+) |
| New REPL commands | 15+ (total ~108+) |
| Protocol support | MCP + ACP + A2A (all three) |
| SWE-bench capability | MCTS + Agentless + ReAct (three strategies) |
| Parallel agent count | 4-8 (configurable, no Docker required) |
| Offline voice | whisper.cpp (39MB-1.5GB models) |
| Cost optimization | Smart routing to cheapest viable model |
