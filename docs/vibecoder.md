...
Configuration is saved to `.vibecoder/hooks.json` (workspace or global).

### Memory & Rules

The **Memory Panel**provides editors for AI rules:

- **Project rules** — `.vibecoder.md` in workspace root (committed alongside code)
- **Global rules** — `~/.vibecoder/rules.md` (personal defaults)
...
| `read_archive_file(path)` | A member of an archive as text |
| `read_archive_file_base64(path)` | A member of an archive as base64 bytes |
| `plan_archive_extraction(path)` | Where an extraction would put things, without writing anything |
| `extract_archive(path, destination?)` | Extract an archive into a folder named after it, and report where the member in `path` landed |
...
| Command | Description |
|---------|-------------|
| `get_vibecoder_rules()` | Load project-level rules (`.vibecoder.md`) |
| `save_vibecoder_rules(content)` | Save project-level rules |
| `get_global_rules()` | Load global rules (`~/.vibecoder/rules.md`) |
...
| Command | Description |
|---------|-------------|
| `get_mcp_servers()` | Load configured MCP servers from `~/.vibecoder/mcp.json` |
...
| Command | Description |
|---------|-------------|
| `record_cost_entry(session_id, provider, model, prompt_tokens, completion_tokens, task_hint?)` | Append cost record to `~/.vibecoder/cost-log.jsonl` with `estimated_cost_usd` pricing |
| `get_cost_metrics()` | Load all entries, compute per-provider aggregates, total cost/tokens, budget remaining |
| `set_cost_limit(limit_usd?)` | Set or clear monthly budget cap (persisted at `~/.vibecoder/cost-config.json`) |
...
| Command | Description |
|---------|-------------|
| `save_arena_vote(vote)` | Persist a blind vote to `~/.vibecoder/arena-votes.json` (provider A/B, winner, prompt, timestamp) |
...
- Auto-loads `*.wasm` files from `~/.vibecoder/extensions/`
...
|----------|-------------|
| `load_workspace_rules(root)` | Load `<workspace>/.vibecoder.md` |
| `load_global_rules()` | Load `~/.vibecoder/rules.md` |
| `combined_rules(root)` | Merge both into a single system-prompt injection |
| `save_workspace_rules(root, content)` | Save project rules |
| `save_global_rules(content)` | Save global rules (creates `~/.vibecoder/` if needed) |
...
VibeCoder implements WCAG 2.1 Level AA accessibility:
...
# End-to-end tests
cd vibecoder/e2e && npm test
```

See [TESTING.md](https://github.com/TuringWorks/vibecody/blob/main/vibecoder/TESTING.md) for manual testing checklist.
...
See [DEBUG.md](https://github.com/TuringWorks/vibecody/blob/main/vibecoder/DEBUG.md) for common troubleshooting steps.
