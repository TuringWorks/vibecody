...
| **N4** ✅ first half shipped 2026-05-18 as "S3" | Wire into `tool_executor` (`run_in_native_sandbox` constructs `vibe_sandbox_native::native()`, calls `bind_rw(workspace)`, sets `NetPolicy::None`/`Direct` from `network_disabled`, spawns `sh -c <cmd>`, collects stdout/stderr via `wait_with_output()`, emits `vibecody::sandbox::spawn { tier, cwd, net, cmd_len }`). Falls back to legacy `CommandExecutor::execute_sandboxed` with a `sandbox.downgrade` warn event when the native backend can't be constructed (bwrap missing, etc.). `vibecoder/src-tauri` half + `with_no_network()`→`with_sandbox(policy)` rename still open. | `vibecli/vibecli-cli/src/tool_executor.rs`, future `vibecoder/src-tauri` | 4 new integration tests in `vibe-sandbox-native/tests/s3_tool_executor_wiring.rs` cover the wiring (constructs native, NetPolicy toggle, .vibecli deny-list on Linux + macOS, OsStr arg shape) — 4/4 pass on macOS. |
| **N5** | Tauri command surface for vibecoder to set sandbox policy per chat tab | `vibecoder/src-tauri/src/commands.rs` | RTL: settings panel toggles enforce on next spawn |
