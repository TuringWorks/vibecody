...
| **F1** ✅ shipped 2026-05-19 (builder + CI publish + cosign + daemon-side manager) | `scripts/build-rootfs-firecracker.sh` is the Docker-based, cross-platform (macOS / Linux / Windows-Docker) builder for the Tier-3 microVM rootfs. Two-stage Docker build: stage 1 installs BusyBox + bash + ca-certificates into `/sysroot` (resolves all BusyBox applets, shims `/bin/sh` → bash, ships musl loader + libc + readline + ncursesw + TLS root certs + `/etc/passwd` + `/etc/group` + provenance markers `/etc/vibe-rootfs-id` / `/etc/vibe-rootfs-built`); stage 2 packs `/sysroot` into a 20 MiB ext4 image via `mke2fs -d` (no loop device, works in Docker Desktop). Output: `target/firecracker-rootfs/rootfs.ext4` + `.sha256` sidecar. Configurable via `ROOTFS_SIZE_MB`, `ROOTFS_OUT_DIR`, `ALPINE_VERSION` env. `Makefile` targets `make rootfs-firecracker` + `make rootfs-firecracker-clean`. 2 integration tests in `vibe-sandbox-firecracker/tests/f1_rootfs_artifact.rs` verify (when the artifact exists) ext4 superblock magic 0xEF53 at byte 1080, ≤ 32 MiB ceiling, `.sha256` sidecar shape; both tests pass on macOS host. **CI publish**: new `build-firecracker-rootfs` job in `.github/workflows/release.yml` runs the builder, executes the artifact-contract tests, **cosign-attests the image via Sigstore keyless OIDC** (`cosign sign-blob --bundle …cosign.bundle`), and uploads to the GitHub Release alongside other artifacts. Release notes call out the new artifacts (`.ext4`, `.ext4.sha256`, `.ext4.cosign.bundle`). **Daemon-side manager** (`vibe-sandbox-firecracker::rootfs::RootfsManager`): verify (ext4 magic + size ceiling + SHA256 match against explicit value or `.sha256` sidecar) + content-addressed install (`rootfs-<sha>.ext4`, atomic .tmp+rename, idempotent) + cached resolver + cosign passthrough. Self-contained streaming SHA256 (no `sha2` dep on this leaf crate); tested against NIST FIPS 180-2 vectors + the real F1 artifact. 19 unit tests + 1 integration test, all pass. | `scripts/build-rootfs-firecracker.sh`, `Makefile`, `.github/workflows/release.yml`, `vibe-sandbox-firecracker/src/rootfs.rs`, `vibe-sandbox-firecracker/tests/f1_rootfs_*.rs` | local: `make rootfs-firecracker` produces 20 MiB image; `cargo test -p vibe-sandbox-firecracker` 22 tests pass; CI: rootfs job runs on every release tag |
| **F2.1** ✅ shipped 2026-05-19 | Firecracker REST API request shapes (`vibe-sandbox-firecracker::api`) — pure-data + serde structs for `/boot-source`, `/machine-config`, `/drives/{id}`, `/vsock`, `/actions` endpoints. `VmConfig::boot_sequence()` walks them in Firecracker-required order (machine-config → boot-source → drives → vsock → InstanceStart). Field naming is snake_case-enforced (Firecracker rejects camelCase). 18 unit tests pin JSON shape, optional-field omission (`cache_type`, `track_dirty_pages`, `boot_args`, `initrd_path`, `vsock`), PascalCase enum encoding (`ActionType::InstanceStart` / `DriveCacheType::Writeback`), boot-sequence ordering invariants, and `ApiRequest.body` shape (object, not string). Sets up F2.2 (the Linux-only HTTP-over-UDS client + microVM process lifecycle) so JSON regressions surface at `cargo test` on any platform, not only on Linux+KVM. | `vibe-sandbox-firecracker/src/api.rs` | unit: 18/18 pass on macOS |
| **F2.2** | Boot-once-per-call: jailer + firecracker + minimal kernel + rootfs + tmpfs overlay; no folder bind, no broker | crate (consumes F2.1 + F1.5) | integration: `bash -c "echo hi"` returns "hi" via stdout |
| **F3.1** ✅ shipped 2026-05-19 | virtio-fs share config + virtiofsd argv builder (`vibe-sandbox-firecracker::virtiofs`). `VirtioFsShare::new(host, tag, socket, ro)` validates host path against the credential-dir deny-list (`.vibecli`/`.vibecoder`/`.claude`/`.ssh`/`.aws`/`.gnupg` segments + `daemon.token`/`profile_settings.db`/`workspace.db`/`id_rsa`/etc. filenames, case-insensitive) and refuses traversal, empty/oversized/garbled mount tags. `argv()` emits the exact virtiofsd CLI: `--socket-path=… --shared-dir=… --cache=auto|always|none [--readonly] [--xattr] --thread-pool-size=2 --announce-submounts`. `kernel_cmdline_fragment(/work)` produces a structured `vibe.virtiofs=<tag>:<mount>:<ro\|rw>` token the in-guest init parses to mount the share. 22 unit tests pin deny-list (segment + filename + case-insensitive + lookalike-accept), tag validation (length, characters, edge cases), argv shape (rw/ro/xattr/cache modes), cmdline fragments, and serde round-trip. Sets up F3.2 (the Linux-only spawn of virtiofsd + Firecracker config-side wiring) so all the policy/argv code is settled before any process spawn. | `vibe-sandbox-firecracker/src/virtiofs.rs` | unit: 22/22 pass on macOS |
| **F2.2-A** ✅ shipped 2026-05-19 | HTTP-over-UDS client (`vibe-sandbox-firecracker::api_client`) for Firecracker's API socket. Pure std (`std::os::unix::net::UnixStream`), sync, tokio-free, no hyper/reqwest dep. Surface: `put_json(socket, path, body, timeout)` + `get(socket, path, timeout)` + `wait_for_socket(path, timeout)` + `ApiResponse{status, body}` + typed `ApiClientError` (connect/write/read/bad-response/http-error/timeout). HTTP/1.1 request frame includes `Host: localhost` + `Content-Type: application/json` + `Content-Length` (exactly body byte count) + `Connection: close`. Response parser handles 204 (no body), 200+JSON body, 400+`fault_message`. Read+write timeouts so a stuck firecracker can't hang the boot sequence forever. `#[cfg(unix)]` gating since UDS isn't on `std::os::unix::net` for Windows targets (consumer F2.2-B is Linux-only anyway). 11 unit tests: status-code parsing (204/200/400/garbage), separator validation, end-to-end PUT via stub `UnixListener` (verifies request line, headers, body round-trip), 400 surfaces as `HttpError{status, body}`, `Content-Length` exactly matches body byte count, `wait_for_socket` returns < 500 ms when socket exists / times out on missing socket, `is_success` boundary at 200/299. Plus a property test that fixes a parallelism bug found during development: socket-path uniqueness across parallel tests now via an `AtomicU64` counter (the original `Instant::elapsed()` always-zero helper caused path collisions under `cargo test`'s default thread count). All 85 vibe-sandbox-firecracker lib tests pass. | `vibe-sandbox-firecracker/src/api_client.rs` | unit: 11/11 pass on macOS via stub UnixListener |
| **F2.2-B** ✅ shipped 2026-05-19 | firecracker + jailer argv builders (`vibe-sandbox-firecracker::process`). `FirecrackerSpawn` builds the `--api-sock` / `--id` / `--log-path` / `--level` / `--config-file` argv with vm-id validation (alphanumeric+dash+underscore, ≤ 64 chars) and the "no `--level` without `--log-path`" invariant Firecracker enforces. `JailerWrap` adds the upstream `jailer` chroot+uid/gid+cgroup wrapper required for production — emits `--id` / `--exec-file` / `--uid` / `--gid` / `--chroot-base-dir` / `--cgroup key=value` pairs / separator `--` / inner firecracker argv with `--id` and `--api-sock` stripped (jailer rewrites those). `LogLevel` enum with FromStr round-trip + invalid-level error. 17 unit tests cover: vm-id validation (empty / slash / too-long / max-64-boundary / dash+underscore-accept), argv shape (minimal, with log path, without log path → no `--level`, with config file, extra args appended last), LogLevel round-trip + bogus-input rejection, jailer required flags + `--` separator presence as standalone arg, jailer post-separator strips `--id` and `--api-sock` but keeps `--log-path` / `--level`, jailer cgroup `key=value` formatting, `program()` accessors. All 101 vibe-sandbox-firecracker lib tests pass. The remaining F2.2-C piece — actually `std::process::Command::spawn` + Child management + boot orchestration via F2.2-A's `wait_for_socket` + `put_json` walked over F2.1's `boot_sequence()` — is the final Linux-only runtime stitch. | `vibe-sandbox-firecracker/src/process.rs` | unit: 17/17 pass on macOS |
| **F2.2-C / F3.2** | Stitch: spawn firecracker (with jailer) + virtiofsd + walk boot_sequence + reap on shutdown | crate (consumes F2.1, F2.2-A, F2.2-B, F3.1) | integration on Linux+KVM: `bash -c "echo hi"` returns "hi" via stdout; write `/work/out`, host sees it |
| **F4.1** ✅ shipped 2026-05-19 | vsock broker bridge config (`vibe-sandbox-firecracker::bridge`) — pure-data types defining the daemon↔broker↔microVM handoff: `BridgeConfig` (sandbox_id, policy_id, guest_cid, host_uds_path, guest_proxy_url, no_proxy list) + `PolicyHandshake` (the JSON the daemon POSTs to the broker's control channel: "attach a bridge for sandbox X with policy Y on UDS Z") + `BridgeAttachResponse` (tagged enum: `ready{sandbox_id, uds_path}` / `refused{sandbox_id, reason}`). Helpers: `as_firecracker_vsock()` converts to F2.1's `Vsock` struct (single source of truth for guest_cid + uds_path); `kernel_env_vars()` emits the upper-and-lowercase `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` env (most clients only check one case); `kernel_cmdline_fragment()` produces `vibe.bridge=<sandbox>:<policy>:<proxy_url>` for the in-guest init. 13 unit tests pin: Firecracker `/vsock` round-trip, handshake JSON shape, env-var case duplication, NO_PROXY-empty omission, BTreeMap-sorted iteration, kernel-cmdline structure, attach-response tagged-enum encoding, and no_proxy `serde(default)` invariant. All 74 vibe-sandbox-firecracker lib tests pass. | `vibe-sandbox-firecracker/src/bridge.rs` | unit: 13/13 pass on macOS |
| **F4.2** | virtio-vsock listener on broker side + in-guest proxy shim; HTTP_PROXY works | `vibe-broker` (vsock listener) + crate (consumes F4.1) | integration: `curl https://example.com` from inside reaches broker, lands in audit log |
...
| **F8** ✅ schema shipped 2026-05-19 | `vibe-sandbox-firecracker::skill_policy::SkillSandboxPolicy` — single typed shape that composes F2.1 (`MachineConfig`) + F3.1 (`VirtioFsShare`[]) + F4.1 (`BridgeConfig`) + extra env (BTreeMap) + extra boot args + wall-clock timeout. `from_json` parses + auto-validates; `validate()` re-runs the credential-dir deny-list on hand-deserialized shares (so a malicious manifest can't bypass `VirtioFsShare::new`), enforces machine bounds (vcpu ∈ 1..=32, memory 32 MiB..=32 GiB), POSIX env-name rules (`[A-Za-z_][A-Za-z0-9_]*`), non-zero wall-clock timeout, ≤ 4096-char kernel cmdline. `into_vm_config(rootfs, kernel)` composes a fully-resolved Firecracker `VmConfig` ready for `boot_sequence()` to walk. `#[serde(deny_unknown_fields)]` so a typo in a skill manifest (e.g. `mahcine` instead of `machine`) fails the load — not the boot. 23 unit tests pin: defaults, empty `{}` parses, unknown-field rejection, machine bounds (zero vcpu / excess vcpu / tiny memory / excessive memory), share deny-list re-validation (hand-built `.aws/credentials` share rejected), env-name rules (dash rejected, digit-start rejected, underscore-start accepted), zero-timeout rejection, into_vm_config composition (defaults applied, boot_args contain console+root, extra_boot_args appended, drive is ro-root, vsock added when bridge set, overlong cmdline rejected), boot_sequence integration emits the expected 5-endpoint walk, full serde round-trip, optional-field omission when None/empty. Skill-manifest integration (the YAML→JSON loader in `vibe-ai/src/skills.rs`) is the daemon-side follow-up — schema + validator + composer are the structural part and ship today. | `vibe-sandbox-firecracker/src/skill_policy.rs` | unit: 23/23 pass on macOS |
