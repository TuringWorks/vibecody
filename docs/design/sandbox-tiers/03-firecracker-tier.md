...
| **F2.2-B / F3.2** | Spawn firecracker + virtiofsd + Firecracker config wiring; folder rw works | crate (consumes F2.1, F2.2-A, F3.1) | integration on Linux+KVM: `bash -c "echo hi"` returns "hi" via stdout; write `/work/out`, host sees it |
