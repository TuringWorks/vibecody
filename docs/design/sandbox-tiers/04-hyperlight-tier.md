...
| **H2/H3** ✅ ABI shipped 2026-05-19 | `vibe-sandbox-hyperlight::host_abi` defines the WASM↔host wire shapes for both slices in one module: **H2 FS** — `FsRequest { Open / Read / Write / Close / ReadDir / Stat }` (tagged `op` field, snake_case) + `FsResponse { Opened / Read / Wrote / Ok / DirEntries / StatResult / Err{errno, message} }` (tagged `result` field) + `FsOpenFlags{read, write, create, truncate}` with `validate()` (rejects `truncate`/`create` without write, rejects neither-read-nor-write) + `DirEntry{name, kind}` + `FileStat{size, kind, readonly, mtime_unix_secs}` + `FileKind { File / Dir / Symlink / Other }`. **H3 egress** — `EgressRequest{method, url, headers, body, timeout_ms}` (snake_case enum tag) with `validate()` (uppercase ASCII method, http/https-only, timeout 1ms..=5min) + `EgressResponse { Ok{status, headers, body} / Denied{reason} / NetworkError{reason} }` distinguishing policy denial (final) from network failure (potentially retryable). Bodies are base64 in JSON (in-tree RFC 4648 encoder/decoder, same rationale as the in-tree SHA-256 — leaf crate stays small). `fn_names` module pins the host-function registry names (`fs.open`, `fs.read`, …, `egress.request`) so a typo at registration time fails the unit test, not the integration. 28 new unit tests cover: open-flags validation (4 cases incl. read-only default + rw+create+truncate accept), FsRequest/Response tagged-enum shape, base64 round-trip + the 7 RFC 4648 §10 reference vectors + invalid-char rejection, EgressRequest validation (empty method, lowercase method, non-http scheme, zero/over-5-min timeout, default 30 s), EgressResponse Ok-round-trip + Denied/NetworkError tag distinguishability, fn_names uniqueness + no-whitespace + no-slash invariants. ABI ships ahead of H1 (the Wasmtime-on-Hyperlight guest binary) so JSON-shape regressions surface at `cargo test` on any platform, not only at runtime on a Linux+KVM CI runner. | `vibe-sandbox-hyperlight/src/host_abi.rs` | unit: 30/30 vibe-sandbox-hyperlight tests pass on macOS |
| **H4** ✅ selector shipped 2026-05-19 | `vibe-extensions::loader` H4: `ExtensionTier` enum (`Wasmtime` / `HyperlightIfAvailable`) + `ExtensionLoader::with_tier_preference()` builder + `effective_tier()` accessor + pure `resolve_tier(pref)` helper + `Extension::tier()` reporter (records which backend actually served the load). `hyperlight_available()` is the host-capability probe — stubbed to `false` until slice H1 lands, so `HyperlightIfAvailable` resolves to `Wasmtime` today; downgrades emit a structured `vibe.extensions.tier.downgrade` trace event so the recap pipeline picks up the fallback decision. Means callers can opt into Hyperlight early without breaking — they get Wasmtime + the trace event until H1 flips the probe. 8 new unit tests pin: default is Wasmtime, resolve is identity for Wasmtime, HyperlightIfAvailable resolves to Wasmtime when `hyperlight_available()=false`, loader `effective_tier()` reflects preference, `Extension::tier()` round-trip through a real load, Display stability for log formatters. All 66 vibe-extensions tests pass. E2E "same module both tiers produces same result" test deferred to H1 (needs the real Hyperlight runtime to compare). | `vibe-extensions/src/loader.rs` | unit: 8/8 pass on macOS |
| **H5** ✅ shipped 2026-05-18 | Tier-1 hardening — Wasmtime `consume_fuel(true)` + `epoch_interruption(true)` wired into `ExtensionLoader`; per-extension fuel + epoch budgets reset on every `try_call`; background `vibe-extensions:epoch-ticker` thread bumps the engine epoch at 100 ms intervals; new typed `ExtensionError::Exhausted { kind: Fuel \| Epoch, name }` lets the daemon log a `vibe.extensions.exhausted` event. Public `Extension::try_call` / `set_fuel` / `set_epoch_deadline` + loader builders `with_default_fuel` / `with_default_epoch_deadline` / `with_interval`. Done independently of H1–H4 since some users will stay on Tier-1. | `vibe-extensions/src/loader.rs` | 4 new unit tests: infinite-loop WAT module is killed by fuel deadline; same module is killed by epoch deadline (10 ms interval, 1-tick budget); per-extension `set_fuel` override beats loader default; `ExhaustionKind::Display` stable for daemon logs. All 58 tests pass. |
| **H6** 🟡 preview shipped 2026-05-19 | `scripts/check-sandbox-tiers.sh` (+ `make sandbox-doctor` / `make sandbox-doctor-json`) is the standalone, daemon-free probe — reports Tier-0/1/2/3 availability + backend identification + actionable remediation hints (`bwrap not installed — apt install bubblewrap`, `/dev/kvm not r/w — usermod -aG kvm`, …) plus Firecracker rootfs presence from the F1 builder. JSON output is the stable contract for downstream tooling; status enum (`ok`/`probable`/`degraded`/`needs-perm`/`missing`/`unsupported`) pinned in `vibe-sandbox/tests/sandbox_doctor_probe.rs`. Lets operators inspect tier availability today without waiting on the `vibecli doctor` integration (still pending — gated on the Metal-toolchain build issue that blocks main.rs edits in dev). | `scripts/check-sandbox-tiers.sh`, `Makefile`, `vibe-sandbox/tests/sandbox_doctor_probe.rs`, future `main.rs` | local: 2/2 JSON-shape tests pass on macOS; `make sandbox-doctor` and `make sandbox-doctor-json` produce expected output |
