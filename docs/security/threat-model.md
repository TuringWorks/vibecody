...
| 21 | ~~`io::Error` displays leak workspace paths to HTTP error bodies~~ — **partial fix 2026-05-13**: added `internal_error` / `internal_error_value` helpers (correlation-ID + server-side log + opaque body), converted the 16-site recap/resume cluster (the sqlx-backed routes that leaked workspace.db paths). Semgrep rule `error-body-leaks-display` (WARNING) blocks new violations. 6 residual sites remain (LLM/stream user-facing errors + a few session/job helpers) — out of scope for the recap-focused sweep; tracked as follow-up before promoting the rule to ERROR. | 5 | 10 | 8 | 5 | 7 | 7.0 | 🟡 partial |
...
| 21 | HTTP error handlers must map `io::Error`/`anyhow::Error` to opaque `{"error":"internal"}` responses | 🟡 partial — helper + 16-site recap/resume cluster done; 6 residual sites + ERROR-level semgrep promotion pending. |
