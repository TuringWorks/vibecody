...
|---|---|---|
| 1 | Mark all T5-derived strings (file contents, web fetch, MCP output) with a `Tainted<T>` wrapper; never permit tainted strings to issue tool calls without an intervening user confirmation step | 🟡 design ready — full proposal in [`tainted-data-flow.md`](./tainted-data-flow.md). Slice A (gate `shell.exec` end-to-end) is the next concrete shippable; ~3 days. |
