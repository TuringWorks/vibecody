...
| 3 | Per-backend sandbox pen-test harness; bwrap profile audit | 🟡 in progress — design in `docs/design/sandbox-tiers/`; **Linux harness scaffold shipped 2026-05-14**: `vibecli/crates/vibe-sandbox-native/tests/pen_test_harness.rs` codifies 20+ attack-scenario tests across 7 categories (path-escape, credential-dir deny-list, env-policy, net-policy, resource limits, broker-socket boundary, bwrap profile regression). Includes a `#[ignore]`d test that documents the known broker-socket-path-not-deny-listed gap. Next: macOS sandbox-exec + Windows AppContainer harnesses. |
