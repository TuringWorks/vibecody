...
| 3 | Per-backend sandbox pen-test harness; bwrap profile audit | 🟡 in progress — design in `docs/design/sandbox-tiers/`. **Linux harness shipped 2026-05-14** (`pen_test_harness.rs`, 20+ tests across 7 categories). **macOS harness shipped 2026-05-14** (`pen_test_harness_macos.rs`, 18+ tests across 5 categories: subpath validation, `.sb` profile contract, NetPolicy→rule mapping, **4 `#[ignore]`d tests documenting the macOS↔Linux deny-list asymmetry** — macOS's `validate_subpath` does not yet have the `.vibecli`/`.vibeui`/`.claude`/`.ssh` deny-list that Linux has; the ignored tests encode the expected behavior so a future PR can un-ignore them as the gap closes, and tier identity). Next: Windows AppContainer harness. |
