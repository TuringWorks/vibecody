...
| 3 | Per-backend sandbox pen-test harness; bwrap profile audit | 🟡 in progress — design in `docs/design/sandbox-tiers/`. **Linux harness shipped 2026-05-14** (`pen_test_harness.rs`, 20+ tests across 7 categories). **macOS harness shipped 2026-05-14** (`pen_test_harness_macos.rs`, 18+ tests across 5 categories incl. 4 `#[ignore]`d tests documenting the macOS↔Linux deny-list asymmetry). **Windows AppContainer harness shipped 2026-05-14** (`pen_test_harness_windows.rs`, 19 tests across 6 categories: path validation, NetPolicy→AppContainer-capability mapping incl. Direct-grants-internetClient and Brokered-does-not, `spawn` slice-N3.2 NotSupported gap pinned, resource-limit omission, **4 `#[ignore]`d deny-list tests** including one Windows-specific test for `AppData\Microsoft\Credentials` / `Vault` paths that should land alongside the cross-platform deny-list port, tier identity). All three backends now covered. Remaining: actually fix the documented deny-list asymmetries (un-ignore the tests by porting the Linux `DENIED_SEGMENTS` to macOS and Windows). |
