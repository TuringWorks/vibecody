...
- **JetBrains Agent Hooks — foundation (P3.10)** — `HookExecutor` service in the JetBrains plugin mirrors the CLI hook protocol (`vibecli-cli/src/hook_abort.rs`): subprocess invocation via `sh -c <command>`, exit-code semantics (0 = allow, 2 = block, other = generic error / non-blocking), structured-JSON-decision stdout override (`{action, reason?, message?}`), 30 s per-hook timeout (timeout → BLOCK to prevent wedged-hook bypass), spawn-failure ALLOW + warning (matches CLI), and ordered-chain short-circuit on the first BLOCK. `HookConfig { name, event, command, enabled }` persists under `VibeCLISettings.State.hooks` via the existing `PersistentStateComponent` infra. Settings UI under **IDE Settings → Tools → VibeCLI** grows a hooks table (Add / Remove via `ToolbarDecorator`, Event column constrained to the seven allow-listed kinds — same set as `plugin_manifest::ALLOWED_HOOK_EVENTS`). First wire: `UserPromptSubmit` fires before `AgentPanel.startAgent` — a BLOCK decision prevents the agent from starting and surfaces the hook's reason inline (`7709bc0b` + `1ebe79c4`). Closes the foundational scope of REMAINING-WORK P3.10; advisory firings for SSE-arriving events and `InlineEditAction` integration are smaller follow-ups.
