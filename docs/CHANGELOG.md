...
- **`/goal` — VS Code ★ pin parity (G13)** — VS Code goals tree closes the cross-surface ★ loop on the editor side: `GoalTreeItem` prefixes the title with ★ for pinned rows and pinned roots sort above the rest so "what am I working on now" lands at the top of the sidebar. New `VibeCLIClient.getPinnedGoalIds(workspace?)` unions the daemon's global pin with the workspace pin (union of two `/v1/goals/current` calls) so a goal pinned from any surface — mobile, watch, VibeUI, CLI — surfaces in the editor. TUI parity is in-flight pending unrelated daemon-side parallel work and will land in a follow-up.
