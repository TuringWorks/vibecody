...
> **Build SDK** — `compileSdk = 37` (Android 17), required by `androidx.lifecycle` 2.11.0; `targetSdk = 36` (Android 16 / Wear OS 6) still governs runtime behaviour and `minSdk = 30` preserves Wear OS 3 compatibility. AGP 9.3.1, Kotlin 2.4.10, Gradle 9.6.1, JDK 17.
...
```bash
curl -LO https://github.com/TuringWorks/vibecody/releases/download/v0.5.11/VibeCody-Wear-v0.5.11.apk
adb connect <watch-ip>:5555          # enable ADB debugging first
adb -s <watch-ip>:5555 install VibeCody-Wear-v0.5.11.apk
```
...
Upload [`VibeCody-Wear-v0.5.11.aab`](https://github.com/TuringWorks/vibecody/releases/download/v0.5.11/VibeCody-Wear-v0.5.11.aab) to the Play Console's internal-testing track. Once approved, install directly from the watch's Play Store.
