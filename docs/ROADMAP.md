...
> This is now the **single canonical roadmap** — earlier iterations (v5, v6) and the 13 deep-dive fit-gap files are absorbed here and in the companion **[Fit-Gap Analysis](./fit-gap-analysis/)**. The original five-competitor delta (Codex CLI, Claude Code, Antigravity, Cursor, Windsurf) is preserved as a historical record below; phases 23–39 are summarised in the **History appendices** at the end of this document. The 2026-04-26 trend snapshot (§1bis) and the v13 audit-reconciliation work (Phase 53, [Appendix D](#appendix-d--phase-53-april-2026-trend-delta--audit-reconciliation)) seeded this layer; the **2026-06-13 trend snapshot (§1quater)** and **Phase 55 ([Appendix F](#appendix-f--phase-55-may-june-2026-delta-c1-c6))** carry the v15 layer, and the **2026-06-22 mid-month refresh (§1quinquies)** is now the freshest — it adds no new gaps (the C1–C6 backlog is unchanged), only frontier-model and ecosystem movement plus one positioning event (the Fable 5 / Mythos 5 export-control suspension).
...
- **SWE-bench refresh (Jun 8–9)** — Claude Mythos 5 + Fable 5 top the provisional weighted-coding board (~100); Opus 4.8 98.9% weighted / 69.2% Pro; GPT-5.4 leads the public set by 4.1 pts but third on commercial code. The 2026-Q1 contamination caveat still stands; §9.3 + Fit-Gap §15.4 are updated with these numbers. **⚠ Superseded 2026-06-22:** Fable 5 + Mythos 5 were suspended by US export-control directive on 2026-06-12 (see §1quinquies) — neither is a quotable production option for any foreign national, so leaderboard-topping ≠ usable; Opus 4.8 is now the highest *available* Anthropic model for VibeCody's default routing.
...
## 1quinquies. June 2026 mid-month refresh (added 2026-06-22)

The nine days since the v15 snapshot (2026-06-13) produced **no new gap class** — the C1–C6 backlog and the A7/B3 escalations are unchanged — but two of them sharpened, the frontier-model board reshuffled hard, and one genuinely novel *positioning* event landed: a frontier model was pulled by government order mid-deployment. This refresh folds those into the v15 layer in place (no version bump, no new Phase). Sources surveyed: [anthropic.com/news](https://www.anthropic.com/news/fable-mythos-access), [fortune.com](https://fortune.com/2026/06/13/anthropic-disables-fable-mythos-export-controls-national-security-threat/), [thenewstack.io](https://thenewstack.io/fable-ban-open-weights/), [blog.modelcontextprotocol.io](https://blog.modelcontextprotocol.io/posts/2026-07-28-release-candidate/), [cursor.com/changelog](https://cursor.com/changelog), [github.blog/changelog](https://github.blog/changelog/), [Gemini Code Assist release notes](https://developers.google.com/gemini-code-assist/resources/release-notes), and the open-weight labs (Z.ai / Moonshot / MiniMax).

### 1quinquies.1 Headline shifts not yet absorbed elsewhere

| Date | Release | What changed | Impact on VibeCody positioning |
|------|---------|--------------|--------------------------------|
| 2026-06-12 | **Fable 5 + Mythos 5 export-control suspension** | US government issued an export-control directive (national-security authority, citing a narrow jailbreak) forcing Anthropic to **disable Fable 5 and Mythos 5 for *all* customers** to comply with a ban on access by any foreign national; both had launched only on 2026-06-09; `claude.ai` access wound down by 2026-06-22 | **Largest positioning event of the quarter for us.** A frontier model topping the board (§1quater.2) became unusable overnight by government order — the textbook case for **provider-agnostic routing + self-hostable + open-weight fallback**. `cost_router.rs` failover and Ollama-first defaults are now a *resilience* story, not just a cost story. Reinforces differentiators §5.2/§5.4/§5.5. "Four open-weight models shipped responses before Anthropic could restore access" — open-weight slot in Counsel defaults is now table-stakes. |
| 2026-06-13 | **GLM-5.2 (Z.ai, 744B)** | Released same day as the v15 snapshot; **now leads the Artificial Analysis Intelligence Index among open weights** (supersedes GLM-5.1 from §1quater) | `useModelRegistry.ts` `zhipu` defaults should track GLM-5.2; `cost_router.rs` re-learns the open-weight ceiling. Trivial c-series close (still *unshipped* — registry only has `glm-4-*`, see §16.7 reconciliation). |
| 2026-06-13 | **Kimi K2.6 → K2.7 Code (Moonshot)** | K2.7 Code coding-specialised variant, ~30% fewer thinking tokens vs K2.6 | Supersedes the K2.6 entry in §1quater; Ollama/Moonshot default target. Note: **no Kimi/Moonshot entry exists in `useModelRegistry.ts` at all yet** — this is a genuine open c-series item, not an append. |
| 2026-06-18 | **Gemini CLI → Antigravity CLI consolidation** | Google stops serving Gemini Code Assist / Gemini CLI requests for individual + AI Pro + AI Ultra tiers on 2026-06-18, unifying its agent tooling into the **Antigravity multi-agent platform (Antigravity CLI)** | The "Gemini CLI" row in §1.1 is now a **legacy/being-retired** entry; the live Google CLI competitor is **Antigravity CLI**. Reinforces C6 (register in the ACP Registry, which Antigravity consumes) and the §1quater Antigravity 2.0 note. |
| 2026-06-23 → 30 (expected) | **Gemini 3.5 Pro GA** | I/O-announced 3.5 Pro hits GA end-June: 2M-token context, Deep Think reasoning | Corrects the v14 "Gemini 4 at I/O" projection further — the family is 3.5, not 4. Add `gemini-3.5-pro` + `gemini-3.5-flash` to `useModelRegistry.ts` (c-series). Both still **absent** today (registry tops out at `gemini-3.1-pro`). |
| 2026-06-10 | **Cursor 3.7 + Bugbot speedup** | Composer 2.5 default; Bugbot now **3× faster, 22% cheaper, +10% bugs found** | Incremental over the §1quater Composer 2.5 / Bugbot note. Reinforces **B3** (always-on review benchmark moves) but opens no new gap. |
| 2026-06-01 | **GitHub Copilot Max tier ($100/mo, 20k credits)** | New individual tier on top of the Jun-1 usage-based AI-Credits billing | Sharpens the "self-hosted, BYO-key, **no per-action metering**" axis further — every suggestion/chat/review now debits credits; a $100 ceiling tier signals heavy-user metering pain. Marketing, not engineering. |
| 2026-05-21 (locked) → 2026-07-28 (final) | **MCP spec RC locked** | The 2026-07-28 RC was **locked 2026-05-21**; 10-week window for Tier-1 SDKs. Concretely: **stateless core removes the `initialize`/`initialized` handshake + `Mcp-Session-Id` header** (protocol version / client info / capabilities now ride a `_meta` field per request); **Tasks extension** = server answers `tools/call` with a task handle, client drives `tasks/get` / `tasks/update` / `tasks/cancel` (server-directed) | Sharpens **C3** from "design" to a concrete conformance target with a deadline. The build is now: drop session pinning in `mcp_streamable.rs`, thread `_meta` per request, add the `tasks/*` verbs. C3 stays P0 in Phase 55. |

### 1quinquies.2 Context-only signals (positioning, not new gaps)

- **Open-weight resilience thesis validated by events** — the Fable/Mythos suspension is the strongest external proof yet for VibeCody's bet: when a hosted frontier model is yanked by regulation, only provider-agnostic + self-hostable + open-weight-capable tooling keeps working. This is now a *sales axis*, not just an architecture preference (§5).
- **Frontier board is volatile** — GLM-5.2 (open) leading the AA index the same week a top closed model (Fable 5) got banned underlines why §9.3 / Fit-Gap §15.4 should quote *available* models, not just leaderboard peaks.
- **Registry reconciliation (verified 2026-06-22 against `useModelRegistry.ts`)** — of the c-series "trivial registry closes" claimed across §1quater + §16.6, **only GPT-5.5 has actually landed**. Opus 4.8, Gemini 3.5 Flash/Pro, DeepSeek V4, Qwen 3.6, Kimi K2.6/K2.7, GLM-5.1/5.2, and MiniMax M3 are **all still absent** from the registry (newest entries are `claude-opus-4-7`, `gemini-3.1-pro`, `glm-4-plus`). The c-series is therefore an **open append-list**, not a closed one — corrected in Fit-Gap §16.7 and the scoreboard footnote.
...
| **Gemini CLI → Antigravity CLI** | Google | Node.js | Gemini-centric, Google Search grounding, long-context — **Gemini CLI retired for individual/Pro/Ultra tiers 2026-06-18; folded into the Antigravity multi-agent platform (Antigravity CLI), see §1quinquies** |
