/**
 * Theme registry + applier — VibeX copy of the canonical VibeUI registry at
 ...
 * prefix (`vibex-*` instead of `vibeui-*`) so theme writes from one app
 ...
 *                          dispatch a `vibex-theme-change` event for any
 *                          downstream listener (Monaco etc. — VibeX has none
 *                          today, but the event is kept for parity).
 */
...
export function applyThemeById(themeId: string): void {
  ...
  localStorage.setItem("vibex-theme-id", theme.id);
  localStorage.setItem("vibex-theme", theme.mode);
  ...
  window.dispatchEvent(new CustomEvent("vibex-theme-change", { detail: { themeId: theme.id, mode: theme.mode } }));
}
