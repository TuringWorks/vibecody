    ...
    <div className="vibex-root">
      <div className="vibex-banner">
        ...
        <span>
          ...
          {daemon.status === "checking" && "VibeX · starting daemon…"}
          {daemon.status === "offline" &&
            `VibeX · daemon offline (${daemon.error ?? daemon.url})`}
        </span>
        {daemon.status === "offline" && (
          <button className="vibex-banner__btn" onClick={startDaemon} disabled={starting}>
