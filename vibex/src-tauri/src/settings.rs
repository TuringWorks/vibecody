//! VibeX settings commands — provider config, API keys, UI prefs, and OAuth
...
pub async fn setting_set(key: String, value: String) -> Result<(), String> {
    store()?.set_provider_config(DEFAULT_PROFILE, "__vibex__", &key, &value)
}

/// Get a single global setting (null if unset).
#[tauri::command]
pub async fn setting_get(key: String) -> Result<serde_json::Value, String> {
    match store()?.get_provider_config(DEFAULT_PROFILE, "__vibex__", &key)? {
        Some(v) => Ok(serde_json::Value::String(v)),
        None => Ok(serde_json::Value::Null),
    }
}

/// Get all VibeX global settings as a JSON object.
#[tauri::command]
pub async fn setting_get_all() -> Result<serde_json::Value, String> {
    store()?.get_all_provider_config(DEFAULT_PROFILE, "__vibex__")
}
