...
4. **Never size an allocation from unvalidated input, and check *before* allocating.** `vec![0u8; content_length]` runs before the body arrives, so a bogus header kills the process without the peer sending a byte — bound it first (`subprocess_dispatch.rs::read_frame` is the template). `as usize` is not validation: a negative `i32` becomes ~`usize::MAX`. On stable Rust an allocation failure **aborts** — there is no `Err` to handle, so the input bound *is* the error handling. And a `Regex::new` outside a `LazyLock` is this codebase's most expensive hot-path mistake: `strip_thinking` cost 27 MB per streamed chunk until they were hoisted. Measure with `crates/vibe-alloc-count`, don't assert.
