...
- **PostToolUse hooks** (`.claude/settings.json`): after any `Edit`/`Write`, automatically runs:
  ...
  - `.ts`/`.tsx` files → `npx --prefix vibecoder tsc --noEmit` (tail 5 lines)
...
vibecoder/src/                ← React/TypeScript frontend (~293 panels + 42 composites)
vibecoder/src-tauri/src/      ← Tauri backend + commands (1,045+ via generate_handler!)
vibecoder/crates/             ← vibe-core, vibe-ai (22 providers), vibe-lsp, vibe-extensions, vibe-collab
