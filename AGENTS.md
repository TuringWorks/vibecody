...
| **Resolve the binary beyond `PATH`.** | A Finder-launched `.app` gets a minimal `PATH`, so `~/.cargo/bin` and Homebrew prefixes must be probed directly or autostart fails for every GUI launch. On Windows the same rule covers `%LOCALAPPDATA%\VibeCody` — `deploy/windows/setup.ps1` installs there and adds it to the *registry* `PATH`, which a running Explorer and everything it launched never see until the user signs out. |
...
| **Detach the session, not just stdio.** `spawn_detached` calls `setsid` on Unix and passes `DETACHED_PROCESS \| CREATE_NEW_PROCESS_GROUP` on Windows. | Measured: a daemon spawned by VibeCoder ran with `PGID` = the *app's* pid, in the app's session. Anything aimed at that group — teardown, `killpg`, a hangup — took the daemon down too, and it exited **cleanly**: no crash report, no panic log, no output. The user saw "daemon offline" and, 30 s later, "back online". On Windows the flags matter for a second reason: the installers ship the daemon as an `externalBin` sidecar, and a GUI-subsystem app spawning that console-subsystem child with no flags gives it a **console window** that stays on screen for the life of the daemon. |
...
| Read, list or extract an archive (ZIP / TAR family) | `vibe_core::archive` — kinds, virtual `archive!/member` paths, bounded reads, zip-slip-safe extraction | a fresh `ZipArchive::new` next to the code that needs it, or shelling out to `unzip`/`tar` |
