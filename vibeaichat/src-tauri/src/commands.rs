//! Tauri commands for the vibeaichat floating window.
...
pub async fn check_daemon(url: String) -> Result<String, String> {
    ...
    if !legacy_ok && service != Some(VIBECLI_SERVICE_NAME) {
        return Err(format!(
            "{url} answered, but it is not the VibeCLI daemon{}. \
             Stop that program, or point VibeAIChat at the right port.",
            service
...
pub async fn start_agent_session(
    ...
    // "chat" (single reply, no tools) or "agent" (tool loop). VibeAIChat is an
    // assistant, so its UI sends "chat"; omitted → the daemon's default.
    mode: Option<String>,
    ...
    // Only sent when it differs from the daemon default, so an older daemon
    // that does not know the field sees a byte-identical request.
    if let Some(m) = &mode {
...
//
// vibeaichat is a lightweight companion overlay (no AiMlComposite, no model
...
// *reachable* from vibeaichat via `invoke()` for any future UI, with the same
...
/// `$HOME` directly (vibeaichat doesn't depend on the `dirs` crate) —
