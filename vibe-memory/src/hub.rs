...
use crate::{GlobalMemStore, HubStats, ProjectMemStore, PurgeReport, SearchResult, StoreKind, MemoryError};
...
impl MemoryContextHub {
    ...
    pub async fn store_to_project(&self, workspace: std::path::PathBuf, content: &str) -> Result<crate::MemoryEntry, MemoryError> {
        let mut guard = self.project_store.write().await;
        if guard.is_none() {
    ...
    pub async fn store_global(&self, content: &str) -> Result<crate::MemoryEntry, MemoryError> {
        if let Some(ref store) = self.global_store {
            ...
            Err(MemoryError::StoreNotFound("Global store not available".to_string()))
        }
    }
    ...
    pub async fn search_context(
        ...
        let global_results = if let Some(ref store) = self.global_store {
            store.search(query, top_k * 2, min_score).await.unwrap_or_default()
        } else {
            Vec::new()
        };
        ...
        debug!(
            ...
            query,
            merged.len(),
            merged.iter().filter(|r| r.store == StoreKind::Project).count(),
            merged.iter().filter(|r| r.store == StoreKind::Global).count(),
        );
    ...
    pub async fn get_stats(&self, workspace: &Path) -> Result<HubStats, MemoryError> {
        ...
        let proj_db_size = std::fs::metadata(project_store.path()).map(|m| m.len()).unwrap_or(0);
        ...
        let global_db_size = if let Some(ref store) = self.global_store {
            std::fs::metadata(store.path()).map(|m| m.len()).unwrap_or(0)
        } else {
            0
        };
...
mod tests {
    ...
    async fn test_hub_basic() {
        ...
        hub.store_to_project(workspace.path().to_path_buf(), "Project knowledge").await.expect("store");
        hub.store_global("Global preference").await.ok(); // May fail in sandbox
        
        let results = hub.search_context(workspace.path(), "knowledge", 5, None).await.expect("search");
    ...
    async fn test_context_assembly() {
        ...
        hub.store_to_project(workspace.path().to_path_buf(), "Rust ownership model").await.expect("store");
        
        let context = hub.assemble_context(workspace.path(), "rust", 2000).await.expect("assemble");
    ...
    async fn test_hub_without_global() {
        ...
        hub.store_to_project(workspace.path().to_path_buf(), "Project only").await.expect("store");
