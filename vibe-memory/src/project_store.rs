...
use crate::{
    classify_sector, default_dimensions, epoch_secs, generate_id,
    extension::ExtensionManager, error::*, schema, MemoryEntry, MemoryMeta,
    SearchResult, StoreKind, Waypoint,
};
...
impl ProjectMemStore {
    pub fn open(workspace: &Path) -> Result<Self> {
        ...
        std::fs::create_dir_all(&vibe_dir).map_err(|e| MemoryError::InvalidWorkspace(e.to_string()))?;
    ...
    pub async fn search(&self, query: &str, top_k: usize, min_score: Option<f64>) -> Result<Vec<SearchResult>> {
        ...
        let rows = stmt.query_map([], |row| {
            ...
            Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?, row.get::<_, String>(2)?,
                row.get::<_, f64>(3)?, row.get::<_, String>(4)?, embedding))
        }).map_err(MemoryError::Sqlite)?;
        
        let mut scored: Vec<_> = Vec::new();
        for row in rows {
            let (id, content, sector, salience, tags, embedding) = row.map_err(MemoryError::Sqlite)?;
        ...
        let results: Vec<SearchResult> = scored
            ...
            .take(top_k)
            .map(|(id, content, sector, score, tags, salience)| {
                SearchResult { id, content, sector, score, salience,
                    tags: serde_json::from_str(&tags).unwrap_or_default(),
                    project_id: None, store: StoreKind::Project }
            })
            .collect();
    ...
    pub async fn search_with_budget(&self, query: &str, top_k: usize, budget_tokens: usize) -> Result<Vec<SearchResult>> {
...
mod tests {
    ...
    async fn test_search() {
        let dir = TempDir::new().unwrap();
        let store = ProjectMemStore::open(dir.path()).expect("open");
        store.store("Rust programming language", None).await.expect("store 1");
        store.store("Python web development", None).await.expect("store 2");
