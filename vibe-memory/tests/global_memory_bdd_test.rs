...
async fn store_computer_level_memory() {
    ...
    let entry = store
        .store("User prefers dark mode and Rust for systems programming", None)
        .await
    ...
    let results = store.search("IDE theme preferences", 5, None).await.expect("search");
...
async fn cross_project_context_retrieval() {
    ...
    store
        .store_from_project("Use dataclasses for better performance in Python", "proj-b", None)
        .await
        .expect("store proj-b");
    ...
    let project_ids: Vec<_> = results.iter()
...
async fn global_store_persists_after_project_deletion() {
    ...
    store.mark_project_deleted("proj-a").await.expect("mark deleted");
    
    // Entry should still exist
    let entry = store.get(&entry_id).await.expect("get");
    assert!(entry.is_some(), "Entry should persist after project deletion");
    
    // Other projects can still access
    let results = store.search("architectural decision", 5, None).await.expect("search");
    assert!(!results.is_empty());
}
...
async fn global_memory_merge_with_project() {
    ...
    let project_store = ProjectMemStore::open(
        &std::env::temp_dir().join("test-merge-project")
    ).expect("open project");
    
    // Setup project memories
    project_store.store("Project-specific API endpoint", None).await.expect("proj store 1");
    project_store.store("Project-specific database schema", None).await.expect("proj store 2");
    
    // Setup global memories
    global_store.store("General best practice: use REST", None).await.expect("global 1");
    global_store.store("General security consideration: validate input", None).await.expect("global 2");
    global_store.store("General naming convention: snake_case", None).await.expect("global 3");
    
    // Create hub and query
    let hub = MemoryContextHub::new();
    let context = hub.assemble_context("api design", 500).await.expect("assemble");
...
async fn computer_level_encryption() {
    ...
    // Store a memory
    store.store("machine-wide preference", None).await.expect("store");
    drop(store);
    ...
    // Store in project
    project_store.store("project-specific data", None).await.expect("store");
    drop(project_store);
    ...
    let proj_path = ProjectMemStore::open(workspace.path()).expect("reopen project").path();
    
    // Path derivation differs
    assert_ne!(global_path, proj_path, "Global and project paths should differ");
}
...
async fn global_store_shared_access() {
    ...
    let id1 = store1.store("Shared memory from process 1", None).await.expect("store").id;
    drop(store1);
    ...
    assert!(entry.is_some(), "Second instance should see entries from first");
    
    // Can add more
    let id2 = store2.store("Shared memory from process 2", None).await.expect("store").id;
...
async fn global_store_project_filtering() {
    ...
    store.store_from_project("Data from project X", "proj-x", None).await.expect("x1");
    store.store_from_project("More from project X", "proj-x", None).await.expect("x2");
    store.store_from_project("Data from project Y", "proj-y", None).await.expect("y1");
    ...
    assert_eq!(proj_x_memories.len(), 2, "Should have 2 memories from proj-x");
...
async fn global_store_sector_stats() {
    ...
    store.store("Yesterday I worked on authentication", None).await.expect("episodic");
    store.store("A fact: Rust prevents null pointer exceptions", None).await.expect("semantic");
    store.store("Step 1: run cargo build", None).await.expect("procedural");
    ...
    assert!(stats.contains_key("procedural"), "Should have procedural count");
...
async fn search_with_sector_filter() {
    ...
    store.store("Yesterday we had a meeting", None).await.expect("episodic");
    store.store("Definition: async means concurrent execution", None).await.expect("semantic");
    store.store("To deploy: run kubectl apply -f config.yaml", None).await.expect("procedural");
    
    // Search with sector filter
    let episodic_only = store.search_filtered("meeting", None, Some("episodic")).await.expect("search episodic");
    let semantic_only = store.search_filtered("definition", None, Some("semantic")).await.expect("search semantic");
    
    // Episodic search should only return episodic
    for result in episodic_only {
        assert_eq!(result.sector, "episodic", "Should only return episodic memories");
    }
    
    // Semantic search should only return semantic
    for result in semantic_only {
        assert_eq!(result.sector, "semantic", "Should only return semantic memories");
    }
}
...
async fn global_store_waypoints() {
    ...
    let wp = waypoints.iter().find(|w| w.dst_id == id2).expect("find waypoint");
