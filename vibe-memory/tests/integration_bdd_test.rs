...
async fn hub_stores_and_retrieves_project_memory() {
    ...
    let results = hub
        .search_context(workspace.path(), "async rust", 5, None)
        .await
        .expect("search");
    assert!(!results.is_empty());
    assert!(results
        .iter()
        .any(|r| r.content.contains("Rust") || r.content.contains("async")));
}
...
async fn hub_respects_context_budget() {
    ...
    let context = hub
        .assemble_context(workspace.path(), "memory", 200)
        .await
        .expect("assemble");
...
async fn hub_applies_sector_weights_to_ranking() {
    ...
    // Search
    let results = hub
        .search_context(workspace.path(), "api meeting build", 5, None)
        .await
        .expect("search");
