...
/// Without it the window loads `http://localhost:1422`, nothing is listening there, and the
    ...
    if release && no_embed {
        println!(
            "cargo:warning=this release build will not embed the frontend — it \
             will load http://localhost:1422 and open on a blank window. Build it with \
             `npm run tauri:build` (or `make build-vibedesk`), or add `--features custom-protocol`."
        );
    }
}
