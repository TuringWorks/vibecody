 ...
 * VibeDesk must NOT contain Cmd+K inline-completion / InlineChat / FIM / ghost-text
 ...
 *
 * Usage: node scripts/check-no-inline-edit.mjs   (run from vibedesk/)
 */
...
if (violations.length > 0) {
  console.error("❌ VX-013: banned inline-edit / Cmd+K patterns found in vibedesk/:\n");
  console.error(violations.join("\n\n"));
  console.error(
    "\nVibeDesk uses conversation+Review and the ⌘. DiffCompleteModal surface only. See pdm/08 §1."
  );
  process.exit(1);
}

console.log("✓ VX-013: no banned inline-edit / Cmd+K patterns in vibedesk/");
