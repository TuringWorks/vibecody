...
interface SettingsViewProps {
  onClose: () => void;
  /**
   * Settings that belong to one shell rather than to all of them — VibeAIChat's
   * daemon URL, say. Kept as a prop so the shared screens stay generic instead
   * of accumulating per-app conditionals.
   */
 ...
 */
export function SettingsView({ onClose, extraTabs = [] }: SettingsViewProps) {
  const [tab, setTab] = useState<Tab>("providers");
  const tabs = [...SHARED_TABS, ...extraTabs.map(({ id, label, icon }) => ({ id, label, icon }))];
