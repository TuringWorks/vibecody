 ...
 *                          dispatch a `vibedesk-theme-change` event for any
 *                          downstream listener (Monaco etc. — VibeDesk has none
...
export function applyThemeById(themeId: string): void {
  ...
  localStorage.setItem("vibedesk-theme-id", theme.id);
  localStorage.setItem("vibedesk-theme", theme.mode);
  ...
  window.dispatchEvent(new CustomEvent("vibedesk-theme-change", { detail: { themeId: theme.id, mode: theme.mode } }));
}
