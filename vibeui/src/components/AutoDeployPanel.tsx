              ...
              return (
                <div key={stage.id} style={{ display: "flex", gap: 0 }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginRight: 14 }}>
                    <div style={{ width: 28, height: 28, borderRadius: "50%", background: statusColor + "22", border: `2px solid ${statusColor}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "var(--font-size-sm)", fontWeight: 700, color: statusColor, flexShrink: 0, marginTop: 12 }}>
                      {idx + 1}
                    </div>
                    {idx < stages.length - 1 && (
                  ...
                  <div style={{ flex: 1, background: "var(--bg-secondary)", border: "1px solid var(--border-color)", borderLeft: `3px solid ${statusColor}`, borderRadius: "var(--radius-sm-alt)", padding: "12px 16px", marginBottom: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: "var(--font-size-sm)", padding: "1px 8px", borderRadius: "var(--radius-sm-alt)", background: statusColor + "22", color: statusColor, marginLeft: "auto" }}>{stage.status}</span>
                      {stage.duration_secs !== null && (
                        <span style={{ fontSize: "var(--font-size-sm)", color: "var(--text-muted)" }}>{stage.duration_secs}s</span>
                      )}
                    </div>
          ...
          {gates.map(gate => {
            ...
            return (
              <div key={gate.id} style={{ background: "var(--bg-secondary)", borderRadius: "var(--radius-sm-alt)", border: "1px solid var(--border-color)", borderLeft: `3px solid ${gateColor}`, padding: "12px 16px" }}>
                ...
                <div style={{ display: "flex", gap: 20, fontSize: "var(--font-size-base)" }}>
                ...
                <tr key={plan.id} style={{ borderBottom: "1px solid var(--border-color)" }}>
                  ...
                  <td style={{ padding: "8px 12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      <span style={{ fontSize: "var(--font-size-xs)", color: "var(--text-muted)" }}>{plan.stages_passed}/{plan.stages_total}</span>
                    </div>
                  </td>
                </tr>
              ))}
