    ...
    <div className="panel-container" style={{ padding: 16, color: "var(--text-primary)", fontFamily: "var(--font-mono)", flex: 1, minHeight: 0, overflowY: "auto" }}>
      ...
      {!loading && tab === "sessions" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {snapshots.map(snap => (
            <div key={snap.session_id} style={{ background: "var(--bg-secondary)", borderRadius: "var(--radius-sm-alt)", border: "1px solid var(--border-color)", padding: "12px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: "var(--font-size-md)", fontWeight: 600 }}>{snap.label ?? snap.session_id.slice(0, 12) + "…"}</span>
                <span style={{ fontSize: "var(--font-size-sm)", padding: "2px 12px", borderRadius: "var(--radius-md)", background: statusColor(snap.status) + "22", color: statusColor(snap.status), marginLeft: "auto" }}>{snap.status}</span>
              </div>
        ...
        <div style={{ maxWidth: 600 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 16 }}>
            <select value={replaySession} onChange={e => setReplaySession(e.target.value)}
              style={{ flex: 1, padding: "8px 12px", borderRadius: "var(--radius-sm)", background: "var(--bg-secondary)", color: "var(--text-primary)", border: "1px solid var(--border-color)", fontSize: "var(--font-size-base)" }}>
              {snapshots.map(snap => (
            ...
            <button className="panel-btn" onClick={runReplay} disabled={replaying || !replaySession}
              style={{ padding: "8px 20px", borderRadius: "var(--radius-sm)", cursor: replaying || !replaySession ? "not-allowed" : "pointer", background: "var(--accent-color)", color: "var(--btn-primary-fg, #fff)", border: "none", fontSize: "var(--font-size-base)", fontWeight: 600, opacity: replaying || !replaySession ? 0.6 : 1 }}>
          ...
          {diffResult && (
            <div style={{ background: "var(--bg-secondary)", borderRadius: "var(--radius-md)", border: "1px solid var(--border-color)", padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                <span style={{ fontSize: "var(--font-size-base)", padding: "2px 12px", borderRadius: "var(--radius-sm-alt)", background: diffResult.identical ? "var(--success-color)22" : "var(--warning-color)22", color: diffResult.identical ? "var(--success-color)" : "var(--warning-color)" }}>
              ...
              {diffResult.diff_lines.length > 0 && (
                <pre style={{ background: "var(--bg-primary)", borderRadius: "var(--radius-sm)", padding: "12px 12px", fontSize: "var(--font-size-sm)", overflow: "auto", maxHeight: 300, margin: 0, color: "var(--text-primary)" }}>
        ...
        <div style={{ maxWidth: 520 }}>
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={traceId} onChange={e => setTraceId(e.target.value)}
              ...
              style={{ width: "100%", padding: "8px 12px", borderRadius: "var(--radius-sm)", background: "var(--bg-secondary)", color: "var(--text-primary)", border: "1px solid var(--border-color)", fontSize: "var(--font-size-base)", boxSizing: "border-box", fontFamily: "var(--font-mono)" }} />
          </div>
          ...
          <button className="panel-btn" onClick={verify} disabled={verifying || !traceId.trim() || !refHash.trim()}
