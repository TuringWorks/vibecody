...
function BaseInput({ label, prefix, value, onChange, valid, mono = true }: {
 ...
 <span style={{ width: 60, fontSize: "var(--font-size-xs)", fontWeight: 700, color: "var(--text-secondary)", flexShrink: 0 }}>{label}</span>
 <span style={{ fontSize: "var(--font-size-sm)", color: "var(--text-secondary)", fontFamily: "var(--font-mono)", flexShrink: 0 }}>{prefix}</span>
 <input value={value} onChange={e => onChange(e.target.value)} spellCheck={false}
 style={{ flex: 1, padding: "4px 8px", fontSize: "var(--font-size-base)", fontFamily: mono ? "var(--font-mono)" : "inherit", background: !valid && value ? "color-mix(in srgb, var(--accent-rose) 8%, transparent)" : "var(--bg-primary)", border: `1px solid ${!valid && value ? "var(--error-color)" : "var(--border-color)"}`, borderRadius: "var(--radius-xs-plus)", color: "var(--text-primary)", outline: "none" }} />
 </div>
 );
}
...
export function NumberBasePanel() {
 ...
 {/* Bit width */}
 {BIT_WIDTHS.map(w => (
 <button key={w} onClick={() => setBits(w)} style={{ padding: "2px 8px", fontSize: "var(--font-size-xs)", borderRadius: "var(--radius-xs-plus)", background: bits === w ? "color-mix(in srgb, var(--accent-blue) 20%, transparent)" : "var(--bg-primary)", border: `1px solid ${bits === w ? "var(--accent-color)" : "var(--border-color)"}`, color: bits === w ? "var(--info-color)" : "var(--text-secondary)", cursor: "pointer" }}>{w}-bit</button>
 ))}
 ...
 <div style={{ fontSize: "var(--font-size-xs)", fontWeight: 700, color: "var(--text-secondary)", marginBottom: 8, letterSpacing: "0.05em" }}>BIT FIELD ({bits}-bit, {signed ? "signed" : "unsigned"})</div>
 ...
 {/* Integer range info */}
 <div style={{ marginTop: 8, padding: "8px 12px", background: "var(--bg-secondary)", borderRadius: "var(--radius-sm)", border: "1px solid var(--border-color)", fontSize: "var(--font-size-sm)", lineHeight: 1.8 }}>
 ...
 <div style={{ fontSize: "var(--font-size-xs)", color: "var(--text-secondary)", marginLeft: 68 }}>Supports 0b…, 0x…, or decimal. Operations use {bits}-bit unsigned.</div>
 </div>
 {bitwiseOps !== null ? (
 ...
 <div style={{ color: "var(--text-secondary)", fontSize: "var(--font-size-base)" }}>Enter valid operands above.</div>
 )}
 </div>
 )}
 ...
 <div style={{ fontSize: "var(--font-size-xs)", color: "var(--text-secondary)", marginTop: 4 }}>float32 value</div>
 </div>
 {/* Breakdown */}
 ...
 <div key={label} style={{ flex: bits2, padding: "8px", background: "var(--bg-secondary)", border: `1px solid ${colour}`, borderRadius: "var(--radius-sm)", minWidth: 0 }}>
 <div style={{ fontSize: 9, fontWeight: 700, color: colour, marginBottom: 2 }}>{label} ({bits2}b)</div>
 <div style={{ fontFamily: "var(--font-mono)", fontSize: "var(--font-size-base)", color: "var(--text-primary)", wordBreak: "break-all" }}>
 ...
 {/* 32-bit view */}
 <div>
 <div style={{ fontSize: "var(--font-size-xs)", fontWeight: 700, color: "var(--text-secondary)", marginBottom: 6 }}>32-BIT LAYOUT</div>
