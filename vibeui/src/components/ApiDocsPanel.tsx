...
const methodStyle = (method: string) => {
 ...
 return { background: c.bg, color: c.color, padding: "2px 6px", borderRadius: 3, fontSize: "var(--font-size-xs)", fontWeight: 700, fontFamily: "var(--font-mono)", flexShrink: 0 as const };
};
...
function SchemaView({ schema, depth = 0 }: { schema: Record<string, unknown>; depth?: number }) {
 ...
 <div key={key} style={{ fontSize: "var(--font-size-sm)", lineHeight: 1.6 }}>
 ...
 return (
 <div style={{ marginLeft: indent, fontSize: "var(--font-size-sm)" }}>
 ...
 return (
 <div style={{ marginLeft: indent, fontSize: "var(--font-size-sm)", color: "var(--text-secondary)" }}>
...
function TryIt({ endpoint, serverUrl }: { endpoint: Endpoint; serverUrl: string }) {
 ...
 {hasBody && (
 <div>
 <div style={{ fontSize: "var(--font-size-sm)", color: "var(--text-secondary)", marginBottom: 3 }}>REQUEST BODY (JSON)</div>
 <textarea
 value={body}
 ...
 borderRadius: "var(--radius-xs-plus)", color: "var(--text-primary)", fontSize: "var(--font-size-sm)",
 ...
 disabled={loading}
 style={{
 marginLeft: "auto", padding: "4px 12px", fontSize: "var(--font-size-sm)",
 background: "var(--accent-color)", color: "var(--text-primary)",
 border: "none", borderRadius: "var(--radius-xs-plus)", cursor: loading ? "wait" : "pointer",
 flexShrink: 0,
 }}
 >
 {loading ? "" : "Send"}
 ...
 fontSize: "var(--font-size-sm)", lineHeight: 1.4, overflow: "auto", maxHeight: 200,
...
function EndpointRow({ endpoint, serverUrl }: { endpoint: Endpoint; serverUrl: string }) {
 ...
 <span style={{ fontFamily: "var(--font-mono)", fontSize: "var(--font-size-base)", flex: 1 }}>{endpoint.path}</span>
 {op.summary && <span style={{ fontSize: "var(--font-size-sm)", color: "var(--text-secondary)", maxWidth: 200, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{op.summary}</span>}
 <span style={{ fontSize: "var(--font-size-base)", color: "var(--text-secondary)" }}>{open ? "" : <ChevronDown size={12} />}</span>
 </div>
 ...
 <div>
 <div style={{ fontSize: "var(--font-size-sm)", fontWeight: 700, color: "var(--text-secondary)", marginBottom: 4 }}>PARAMETERS</div>
 {op.parameters.map((p) => (
 <div key={p.name} style={{ display: "flex", gap: 8, fontSize: "var(--font-size-sm)", lineHeight: 1.7 }}>
 ...
 {p.required && <span style={{ color: "var(--text-danger)", fontSize: "var(--font-size-xs)" }}>required</span>}
 ...
 <div>
 <div style={{ fontSize: "var(--font-size-sm)", fontWeight: 700, color: "var(--text-secondary)", marginBottom: 4 }}>REQUEST BODY</div>
 {Object.entries(op.requestBody.content).map(([ct, body]) => (
 <div key={ct}>
 <span style={{ fontSize: "var(--font-size-xs)", color: "var(--text-secondary)", fontFamily: "var(--font-mono)" }}>{ct}</span>
 ...
 {op.responses && (
 <div>
 <div style={{ fontSize: "var(--font-size-sm)", fontWeight: 700, color: "var(--text-secondary)", marginBottom: 4 }}>RESPONSES</div>
 {Object.entries(op.responses).map(([code, resp]) => (
 <div key={code} style={{ display: "flex", gap: 8, fontSize: "var(--font-size-sm)", lineHeight: 1.7 }}>
 <span style={{
...
export function ApiDocsPanel({ workspacePath }: ApiDocsPanelProps) {
 ...
 {error && <div style={{ marginTop: 6, fontSize: "var(--font-size-sm)", color: "var(--text-danger)" }}> {error}</div>}
 </div>
 ...
 style={{
 padding: "3px 8px", fontSize: "var(--font-size-xs)", borderRadius: 3,
 ...
 <div style={{ fontSize: "var(--font-size-sm)", fontWeight: 700, color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>
 {tag}
 </div>
 {eps.map((ep, i) => (
 ...
 <div style={{ fontSize: "var(--font-size-md)" }}>Load an OpenAPI / Swagger spec to get started.</div>
 <div style={{ fontSize: "var(--font-size-sm)", textAlign: "center", lineHeight: 1.6, maxWidth: 280 }}>
