...
function JwtTool() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {token && !parsed && (
        <div style={S.error}>⚠️ Invalid JWT format (expected 3 dot-separated parts)</div>
      )}
      {parsed && (
        <>
          {expiry && (
...
function TimestampTool() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {value && !parsed && <div style={S.error}>⚠️ Cannot parse "{value}"</div>}
    </div>
  );
}
...
function UrlTool() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {[
        ["Encoded", encoded],
        ["Decoded", decoded ?? "⚠️ Invalid percent-encoding"],
      ].map(([label, val]) => (
        <div key={label as string}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
            <span style={S.label}>{label as string}</span>
