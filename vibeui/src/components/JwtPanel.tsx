...
function JsonView({ json }: JsonViewProps) {
 ...
 {parts.map((p, i) => {
 if (/^"[^"]*"\s*:$/.test(p)) return <span key={i} style={{ color: "#89b4fa" }}>{p}</span>;
 if (/^"/.test(p)) return <span key={i} style={{ color: "#a6e3a1" }}>{p}</span>;
 if (p === "true" || p === "false") return <span key={i} style={{ color: "#fab387" }}>{p}</span>;
 if (p === "null") return <span key={i} style={{ color: "#f38ba8" }}>{p}</span>;
 if (/^-?\d/.test(p)) return <span key={i} style={{ color: "#f9e2af" }}>{p}</span>;
...
export function JwtPanel() {
 ...
 {decodeError && <div style={{ padding: "4px 12px", fontSize: 11, color: "#f38ba8", background: "rgba(243,139,168,0.06)" }}>{decodeError}</div>}
 </div>

 {/* Header */}
 {header && (
 <div style={{ borderBottom: "1px solid var(--border-color)" }}>
 <div style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, color: "#89b4fa", background: "var(--bg-secondary)", display: "flex", justifyContent: "space-between" }}>
 ...
 {payload && (
 <div style={{ borderBottom: "1px solid var(--border-color)" }}>
 <div style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, color: "#a6e3a1", background: "var(--bg-secondary)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
 <span>PAYLOAD</span>
 ...
 <span style={{ color: "#89b4fa" }}>{k}</span> = {fmtTs((payload as Record<string, unknown>)[k] as number)}
 </div>
 ))}
 </div>
 )}
 ...
 {signature && (
 <div>
 <div style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, color: "#f38ba8", background: "var(--bg-secondary)", display: "flex", justifyContent: "space-between" }}>
 <span>SIGNATURE</span>
 ...
 <div style={{ padding: "8px 12px", fontFamily: "monospace", fontSize: 11, color: "#f38ba8", wordBreak: "break-all", background: "var(--bg-primary)" }}>{signature}</div>
 ...
 {/* Header editor */}
 <div style={{ borderBottom: "1px solid var(--border-color)" }}>
 <div style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, color: "#89b4fa", background: "var(--bg-secondary)" }}>HEADER (JSON)</div>
 ...
 {/* Payload editor */}
 <div style={{ borderBottom: "1px solid var(--border-color)" }}>
 <div style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, color: "#a6e3a1", background: "var(--bg-secondary)" }}>PAYLOAD (JSON)</div>
 ...
 <button onClick={handleSign} disabled={signing} style={{ padding: "4px 14px", fontSize: 11, fontWeight: 700, background: "rgba(99,102,241,0.2)", border: "1px solid #6366f1", borderRadius: 4, color: "#89b4fa", cursor: "pointer" }}>
 {signing ? "Signing…" : "Sign"}
 </button>
 </div>
 {signError && <div style={{ padding: "6px 12px", fontSize: 11, color: "#f38ba8", background: "rgba(243,139,168,0.06)", borderBottom: "1px solid var(--border-color)" }}>{signError}</div>}
 {/* Generated JWT */}
 {generated && (
 <div>
 <div style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, color: "#cba6f7", background: "var(--bg-secondary)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
 ...
 <div style={{ padding: "10px 12px", fontFamily: "monospace", fontSize: 11, color: "#cba6f7", wordBreak: "break-all", lineHeight: 1.8, background: "var(--bg-primary)" }}>
 ...
 <td style={{ padding: "6px 10px", fontFamily: "monospace", color: "#89b4fa", fontWeight: 700, whiteSpace: "nowrap" }}>{k}</td>
 ...
 <strong style={{ color: "#fab387" }}>Security note:</strong>JWT payloads are Base64-encoded, <em>not encrypted</em>. Anyone who holds the token can read its claims. Only put non-sensitive data in JWT payloads, and always validate the signature server-side before trusting any claim.
 </div>
 </div>
 )}
