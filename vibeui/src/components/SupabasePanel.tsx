    ...
    <div style={s.panel}>
      <div style={s.header}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <span style={{ fontSize: "11px", padding: "2px 6px", borderRadius: "10px", background: connected ? "#1a5c28" : "#5c1a1a", color: connected ? "#4caf50" : "#f44336" }}>
      ...
      {error && <div style={{ padding: "8px 12px", background: "rgba(244,67,54,0.1)", color: "#f44336", fontSize: "12px" }}>{error}</div>}
      ...
      {connected && (
        <>
          ...
          <div style={{ padding: "8px 12px", borderTop: "1px solid var(--border-color)", display: "flex", gap: "8px" }}>
            <button style={{ ...s.btn, background: "#c0392b" }} onClick={() => { setConnected(false); setTables([]); invoke("save_supabase_config", { workspacePath, url: "", anonKey: "" }).catch(() => {}); }}>Disconnect</button>
          </div>
        </>
      )}
    </div>
