...
function InfoRow({ label, value, mono = true, colour }: { label: string; value: string; mono?: boolean; colour?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", borderBottom: "1px solid rgba(255,255,255,0.04)", padding: "5px 12px", gap: 10 }}>
      ...
      <span style={{ flex: 1, fontFamily: mono ? "monospace" : "inherit", fontSize: 12, color: colour ?? "var(--text-primary)", wordBreak: "break-all" }}>{value}</span>
      <CopyBtn text={value} />
    </div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        ...
        {subTab === "calc" && (
          <div>
            {!info && cidrInput.trim() && (
              <div style={{ padding: "8px 12px", fontSize: 11, color: "#f38ba8" }}>⚠ Invalid CIDR. Format: x.x.x.x/prefix (e.g. 192.168.1.0/24)</div>
            )}
            {info && (
              <div>
                ...
                <div style={{ padding: "10px 12px", borderTop: "1px solid var(--border-color)" }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "var(--text-muted)", marginBottom: 8, letterSpacing: "0.05em" }}>
                    ...
                    <span style={{ color: "#89b4fa", fontWeight: 400 }}>■ network bits</span>
                    <span style={{ color: "#a6e3a1", fontWeight: 400, marginLeft: 8 }}>■ host bits</span>
                  </div>
                  {[
                ...
                <div style={{ padding: "6px 12px", borderTop: "1px solid var(--border-color)", fontSize: 11, color: "var(--text-muted)" }}>
                  {PRIVATE_RANGES.some(r => {
                    ...
                    ? <span style={{ color: "#fab387" }}>⚠ This range falls within a private (RFC 1918 / reserved) address space.</span>
                    : <span style={{ color: "#a6e3a1" }}>✓ Public address space.</span>
                  }
                </div>
              </div>
            )}
        ...
        {subTab === "split" && (
          <div style={{ display: "flex", flexDirection: "column" }}>
            {info && (
              <>
                <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--border-color)", background: "var(--bg-secondary)", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <span style={{ fontSize: 11, color: "var(--text-muted)" }}>Split into /<span style={{ color: "#89b4fa", fontFamily: "monospace" }}>{splitPrefix}</span> blocks</span>
                  ...
                  {splitResults.length === 0
                    ...
                    : splitResults.map((s, i) => (
                      <div key={i} style={{ display: "flex", gap: 8, padding: "5px 12px", borderBottom: "1px solid rgba(255,255,255,0.04)", alignItems: "center", fontSize: 11 }}>
                        ...
                        <span style={{ fontFamily: "monospace", color: "#89b4fa", width: 160, flexShrink: 0 }}>{s.cidr}</span>
                        <span style={{ color: "#a6e3a1", width: 110, flexShrink: 0 }}>first: {s.firstHost}</span>
                        <span style={{ color: "#f38ba8", flex: 1 }}>last: {s.lastHost}</span>
                        <CopyBtn text={s.cidr} />
                      </div>
                    ))
                  }
            ...
            {!info && cidrInput.trim() && <div style={{ padding: 12, color: "#f38ba8", fontSize: 12 }}>Fix the CIDR input above first.</div>}
          </div>
        )}
        ...
        {subTab === "reference" && (
          <div style={{ padding: "12px", display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {/* Private ranges */}
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#89b4fa", marginBottom: 8 }}>PRIVATE & RESERVED RANGES (IPv4)</div>
                ...
                <tbody>
                  {PRIVATE_RANGES.map(r => (
                    <tr key={r.range} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", cursor: "pointer" }} onClick={() => { setCidrInput(r.range); setSubTab("calc"); }}>
                      <td style={{ padding: "5px 8px", fontFamily: "monospace", color: "#fab387" }}>{r.range}</td>
            ...
            {/* Cloud defaults */}
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#a6e3a1", marginBottom: 8 }}>CLOUD & CONTAINER DEFAULTS</div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                <thead>
                  <tr style={{ background: "var(--bg-secondary)" }}>
                ...
                <tbody>
                  {CLOUD_DEFAULTS.map((r, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", cursor: "pointer" }} onClick={() => { setCidrInput(r.cidr); setSubTab("calc"); }}>
                      <td style={{ padding: "5px 8px", color: "#cba6f7", fontWeight: 700 }}>{r.provider}</td>
                      <td style={{ padding: "5px 8px", fontFamily: "monospace", color: "#89b4fa" }}>{r.cidr}</td>
            ...
            {/* Prefix cheat-sheet */}
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#fab387", marginBottom: 8 }}>PREFIX LENGTH CHEAT SHEET</div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                <thead>
                  <tr style={{ background: "var(--bg-secondary)" }}>
                ...
                <tbody>
                  {PREFIX_TABLE.map(([prefix, hosts, mask, notes]) => (
                    <tr key={prefix} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <td style={{ padding: "5px 8px", fontFamily: "monospace", fontWeight: 700, color: "#fab387" }}>{prefix}</td>
                      <td style={{ padding: "5px 8px", fontFamily: "monospace", color: "#a6e3a1" }}>{hosts}</td>
