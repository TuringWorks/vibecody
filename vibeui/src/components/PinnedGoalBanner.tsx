 ...
 * `vibeui:pin-changed` window event so the GoalPanel can poke this
...
export function PinnedGoalBanner({ workspacePath }: PinnedGoalBannerProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('vibeui:pin-changed', handler);
    return () => {
      clearInterval(interval);
      window.removeEventListener('vibeui:pin-changed', handler);
    };
  }, [refresh]);
