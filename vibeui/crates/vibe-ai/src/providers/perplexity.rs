...
impl PerplexityProvider {
    ...
    fn api_key(&self) -> Result<&str> {
        self.config.api_key.as_deref().context("Perplexity API key not set (PERPLEXITY_API_KEY)")
    }
...
impl AIProvider for PerplexityProvider {
    ...
    async fn chat_response(&self, messages: &[Message], context: Option<String>) -> Result<CompletionResponse> {
        ...
        openai_compat::send_chat_request(&self.client, &self.chat_url(), api_key, &request, "Perplexity").await
    }
    ...
    async fn stream_chat(&self, messages: &[Message]) -> Result<CompletionStream> {
        ...
        openai_compat::send_stream_request(&self.client, &self.chat_url(), api_key, &request, "Perplexity").await
    }
...
mod tests {
    ...
    #[test]
    fn request_sonar_deep_research_model() {
        let req = ChatRequest {
            model: "sonar-deep-research".into(),
            messages: vec![
                ChatMessage { role: "system".into(), content: "research thoroughly".into() },
                ChatMessage { role: "user".into(), content: "investigate this".into() },
            ],
    ...
    #[test]
    fn request_sonar_reasoning_model() {
        let req = ChatRequest {
