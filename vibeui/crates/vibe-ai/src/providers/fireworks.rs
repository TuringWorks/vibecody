...
impl FireworksProvider {
    ...
    fn build_messages(&self, messages: &[Message], context: Option<String>) -> Vec<FireworksMessage> {
        let mut result: Vec<FireworksMessage> = messages.iter().map(|m| FireworksMessage {
...
impl AIProvider for FireworksProvider {
    ...
    async fn chat_response(&self, messages: &[Message], context: Option<String>) -> Result<CompletionResponse> {
        let api_key = self.config.api_key.as_ref().context("Fireworks AI API key not set (FIREWORKS_API_KEY)")?;
        ...
        let resp = self.client.post(&url)
            ...
            .json(&request)
            .send().await.context("Fireworks AI request failed")?;

        if !resp.status().is_success() {
            let err = resp.text().await?;
            anyhow::bail!("Fireworks AI API error: {}", err);
        }
        let body: FireworksResponse = resp.json().await.context("Failed to parse Fireworks AI response")?;
    ...
    async fn stream_chat(&self, messages: &[Message]) -> Result<CompletionStream> {
        let api_key = self.config.api_key.as_ref().context("Fireworks AI API key not set")?;
        let request = FireworksRequest {
            model: self.config.model.clone(),
        ...
        let resp = self.client.post(&url)
            .header("Authorization", format!("Bearer {}", api_key))
            .json(&request)
            .send().await.context("Fireworks AI stream request failed")?;
        ...
        let stream = resp.bytes_stream().map(|chunk| {
            ...
            for line in text.lines() {
                if let Some(data) = line.strip_prefix("data: ") {
                    ...
                    if let Ok(r) = serde_json::from_str::<FireworksStreamResponse>(data) {
...
mod tests {
    ...
    #[test]
    fn fireworks_request_serde_minimal() {
        let req = FireworksRequest {
            model: "accounts/fireworks/models/llama-v3p1-70b-instruct".into(),
            messages: vec![FireworksMessage { role: "user".into(), content: "test".into() }],
            temperature: None,
            max_tokens: None,
            stream: false,
        };
    ...
    #[test]
    fn fireworks_request_serde_full() {
        let req = FireworksRequest {
            model: "accounts/fireworks/models/llama-v3p1-70b-instruct".into(),
            messages: vec![
                FireworksMessage { role: "system".into(), content: "sys".into() },
                FireworksMessage { role: "user".into(), content: "usr".into() },
            ],
    ...
    #[test]
    fn fireworks_response_deser_no_usage() {
    ...
    #[test]
    fn fireworks_stream_response_deser() {
        let json = r#"{"choices":[{"delta":{"content":"token"}}]}"#;
        let resp: FireworksStreamResponse = serde_json::from_str(json).unwrap();
        assert_eq!(resp.choices[0].delta.content.as_ref().unwrap(), "token");
    }

    #[test]
    fn fireworks_stream_response_deser_null_content() {
    ...
    #[test]
    fn fireworks_message_deser_roundtrip() {
        let msg = FireworksMessage { role: "user".into(), content: "hello world".into() };
        let json = serde_json::to_string(&msg).unwrap();
        let msg2: FireworksMessage = serde_json::from_str(&json).unwrap();
        assert_eq!(msg.role, msg2.role);
        assert_eq!(msg.content, msg2.content);
    }

    #[test]
    fn fireworks_response_deser_multiple_choices() {
    ...
    #[test]
    fn fireworks_response_deser_large_token_counts() {
    ...
    #[test]
    fn fireworks_stream_response_deser_empty_string_content() {
    ...
    #[test]
    fn fireworks_request_mixtral_model() {
        let req = FireworksRequest {
            model: "accounts/fireworks/models/mixtral-8x7b-instruct".into(),
            messages: vec![
                FireworksMessage { role: "system".into(), content: "be helpful".into() },
                FireworksMessage { role: "user".into(), content: "solve this".into() },
            ],
    ...
    #[test]
    fn fireworks_request_streaming_model() {
        let req = FireworksRequest {
            model: "accounts/fireworks/models/llama-v3p1-8b-instruct".into(),
            messages: vec![FireworksMessage { role: "user".into(), content: "hi".into() }],
            temperature: None,
            max_tokens: None,
            stream: true,
        };
