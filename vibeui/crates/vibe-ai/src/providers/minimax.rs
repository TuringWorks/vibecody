//! MiniMax provider — Chinese AI large models, OpenAI-compatible API.
//!
//! Supported models: abab6.5s-chat, abab6.5-chat, abab5.5-chat, MiniMax-Text-01
...
/// MiniMax provider — Chinese AI large models with OpenAI-compatible API.
...
impl MiniMaxProvider {
    ...
    fn base_url(&self) -> String {
        self.config.api_url.clone().unwrap_or_else(|| MINIMAX_BASE_URL.to_string())
    }

    fn build_messages(&self, messages: &[Message], context: Option<String>) -> Vec<MiniMaxMessage> {
        let mut result: Vec<MiniMaxMessage> = messages.iter().map(|m| MiniMaxMessage {
...
impl AIProvider for MiniMaxProvider {
    ...
    async fn chat_response(&self, messages: &[Message], context: Option<String>) -> Result<CompletionResponse> {
        let api_key = self.config.api_key.as_ref().context("MiniMax API key not set (MINIMAX_API_KEY)")?;
        ...
        let resp = self.client.post(&url)
            ...
            .json(&request)
            .send().await.context("MiniMax request failed")?;

        if !resp.status().is_success() {
            let err = resp.text().await?;
            anyhow::bail!("MiniMax API error: {}", err);
        }
        let body: MiniMaxResponse = resp.json().await.context("Failed to parse MiniMax response")?;
    ...
    async fn stream_chat(&self, messages: &[Message]) -> Result<CompletionStream> {
        let api_key = self.config.api_key.as_ref().context("MiniMax API key not set")?;
        let request = MiniMaxRequest {
            model: self.config.model.clone(),
        ...
        let resp = self.client.post(&url)
            .header("Authorization", format!("Bearer {}", api_key))
            .json(&request)
            .send().await.context("MiniMax stream request failed")?;
        ...
        let stream = resp.bytes_stream().map(|chunk| {
            ...
            for line in text.lines() {
                if let Some(data) = line.strip_prefix("data: ") {
                    ...
                    if let Ok(r) = serde_json::from_str::<MiniMaxStreamResponse>(data) {
...
mod tests {
    ...
    fn base_url_constant() {
        assert_eq!(MINIMAX_BASE_URL, "https://api.minimax.chat/v1");
    }

    #[test]
    fn minimax_response_deser() {
    ...
    async fn availability_tracks_api_key() {
        let p_with = MiniMaxProvider::new(test_config());
        assert!(p_with.is_available().await);

        let mut cfg = test_config();
        cfg.api_key = None;
        let p_without = MiniMaxProvider::new(cfg);
    ...
    #[test]
    fn minimax_request_serde_minimal() {
        let req = MiniMaxRequest {
            model: "abab6.5s-chat".into(),
            messages: vec![MiniMaxMessage { role: "user".into(), content: "test".into() }],
            temperature: None,
            max_tokens: None,
            stream: false,
        };
        ...
        assert_eq!(val["model"], "abab6.5s-chat");
    ...
    #[test]
    fn minimax_request_serde_full() {
        let req = MiniMaxRequest {
            model: "abab6.5-chat".into(),
            messages: vec![
                MiniMaxMessage { role: "system".into(), content: "sys".into() },
                MiniMaxMessage { role: "user".into(), content: "usr".into() },
            ],
    ...
    #[test]
    fn minimax_response_deser_no_usage() {
    ...
    #[test]
    fn minimax_stream_response_deser() {
        let json = r#"{"choices":[{"delta":{"content":"token"}}]}"#;
        let resp: MiniMaxStreamResponse = serde_json::from_str(json).unwrap();
        assert_eq!(resp.choices[0].delta.content.as_ref().unwrap(), "token");
    }

    #[test]
    fn minimax_stream_response_deser_null_content() {
    ...
    #[test]
    fn minimax_message_deser_roundtrip() {
        let msg = MiniMaxMessage { role: "user".into(), content: "hello world".into() };
        let json = serde_json::to_string(&msg).unwrap();
        let msg2: MiniMaxMessage = serde_json::from_str(&json).unwrap();
        assert_eq!(msg.role, msg2.role);
        assert_eq!(msg.content, msg2.content);
    }

    #[test]
    fn minimax_response_deser_multiple_choices() {
    ...
    #[test]
    fn minimax_response_deser_large_token_counts() {
    ...
    #[test]
    fn minimax_stream_response_deser_empty_string_content() {
    ...
    #[test]
    fn minimax_request_text01_model() {
        let req = MiniMaxRequest {
            model: "MiniMax-Text-01".into(),
            messages: vec![
                MiniMaxMessage { role: "system".into(), content: "think step by step".into() },
                MiniMaxMessage { role: "user".into(), content: "solve this".into() },
            ],
    ...
    #[test]
    fn minimax_request_abab55_model() {
        let req = MiniMaxRequest {
            model: "abab5.5-chat".into(),
            messages: vec![MiniMaxMessage { role: "user".into(), content: "hi".into() }],
            temperature: None,
            max_tokens: None,
            stream: true,
        };
