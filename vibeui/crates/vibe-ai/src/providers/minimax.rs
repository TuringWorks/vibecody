...
impl MiniMaxProvider {
    pub fn new(config: ProviderConfig) -> Self {
        let display_name = format!("MiniMax ({})", config.model);
        Self {
            config,
    ...
    fn api_key(&self) -> Result<&str> {
        self.config.api_key.as_deref().context("MiniMax API key not set (MINIMAX_API_KEY)")
    }
...
impl AIProvider for MiniMaxProvider {
    ...
    async fn chat_response(&self, messages: &[Message], context: Option<String>) -> Result<CompletionResponse> {
        ...
        openai_compat::send_chat_request(&self.client, &self.chat_url(), api_key, &request, "MiniMax").await
    }
    ...
    async fn stream_chat(&self, messages: &[Message]) -> Result<CompletionStream> {
        ...
        openai_compat::send_stream_request(&self.client, &self.chat_url(), api_key, &request, "MiniMax").await
    }
...
mod tests {
    ...
    fn minimax_request_text01_model() {
        let req = ChatRequest {
            ...
            messages: vec![
                ChatMessage { role: "system".into(), content: "think step by step".into() },
