//! Workspace and global AI rules for VibeCoder.
//!
//! - Project rules: `<workspace>/.vibecoder.md` (committed alongside code)
//! - Global rules:  `~/.vibecoder/rules.md`   (personal defaults)

use std::path::{Path, PathBuf};

const WORKSPACE_RULES_FILE: &str = ".vibecoder.md";
const GLOBAL_RULES_DIR: &str = ".vibecoder";
...
/// Load project-level rules from `<workspace>/.vibecoder.md`.
...
/// Load global rules from `~/.vibecoder/rules.md`.
...
/// Save project rules to `<workspace>/.vibecoder.md`.
...
/// Save global rules to `~/.vibecoder/rules.md` (creates directory if needed).
...
mod tests {
    ...
    fn combined_rules_both_empty() {
        let dir = std::env::temp_dir().join("vibecoder_test_empty_rules");
        let _ = std::fs::create_dir_all(&dir);
        // No .vibecoder.md file → empty workspace rules
    ...
    fn combined_rules_workspace_only() {
        let dir = std::env::temp_dir().join("vibecoder_test_ws_rules");
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join(".vibecoder.md"), "Always use Rust").unwrap();
        let out = combined_rules(&dir);
        assert!(out.contains("## Project AI Rules"));
    ...
    fn save_and_load_workspace_rules_roundtrip() {
        let dir = std::env::temp_dir().join("vibecoder_test_roundtrip");
        let _ = std::fs::create_dir_all(&dir);
    ...
    fn load_workspace_rules_missing_file() {
        let dir = std::env::temp_dir().join("vibecoder_test_no_rules_file");
        let _ = std::fs::create_dir_all(&dir);
        // Ensure file doesn't exist
        let _ = std::fs::remove_file(dir.join(".vibecoder.md"));
    ...
    fn combined_rules_has_section_headers() {
        let dir = std::env::temp_dir().join("vibecoder_test_headers");
        let _ = std::fs::create_dir_all(&dir);
        std::fs::write(dir.join(".vibecoder.md"), "project rule").unwrap();
        let out = combined_rules(&dir);
        assert!(out.contains("## Project AI Rules\n"));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn global_rules_path_is_under_home() {
        let path = global_rules_path();
        assert!(path.to_string_lossy().contains(".vibecoder"));
