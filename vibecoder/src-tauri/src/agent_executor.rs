//! `ToolExecutorTrait` implementation for the VibeCoder Tauri backend.
//!
...
impl TauriToolExecutor {
    ...
    /// continue to propagate to the VibeCoder frontend.
        ...
        let provider = match &self.provider {
            ...
            None => {
                return ToolResult::err(
                    "spawn_agent",
                    "No LLM provider configured for sub-agent spawning in VibeCoder.",
                )
            }
        };
...
mod tests {
    ...
    async fn execute_call_read_file_missing() {
        ...
        let call = ToolCall::ReadFile {
            path: "/tmp/nonexistent_vibecoder_test_file_xyz".into(),
        };
        let result = exec.execute_call(&call).await;
        assert!(!result.success);
    }
