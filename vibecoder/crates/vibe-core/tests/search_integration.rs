...
fn search_finds_matches_across_multiple_files() {
    ...
    assert!(
        results.len() >= 2,
        "unwrap_value appears in main.rs and lib.rs; got {} results",
        results.len()
    );
...
fn search_returns_empty_for_no_match() {
    let dir = setup_project();
    let results =
        search_files(&dir.path().to_path_buf(), "xyzzy_nonexistent_token", false).unwrap();
...
fn search_case_insensitive_finds_mixed_case() {
    ...
    assert!(
        results.iter().any(|r| r.path.contains("README")),
...
fn search_case_sensitive_misses_wrong_case() {
    ...
    assert!(
        results.iter().all(|r| !r.path.contains("README.md")),
...
fn search_results_include_line_number_and_content() {
    ...
    assert!(
        main_result.line_content.contains("fn main"),
