...
//! surfaces:
//!
//! * Tauri commands invoked by the WebView (`vibecoder/src-tauri/...`)
...
mod tests {
    ...
    #[test]
    fn rejects_dot_vibecoder_segment() {
        let err = reject_sensitive_path("/tmp/.vibecoder/api_keys.json").unwrap_err();
        assert!(err.contains(".vibecoder"), "got: {err}");
    }
