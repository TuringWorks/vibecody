...
export function applyThemeById(themeId: string): void {
  ...
  localStorage.setItem("vibecoder-theme-id", theme.id);
  localStorage.setItem("vibecoder-theme", theme.mode);
  ...
  window.dispatchEvent(new CustomEvent("vibecoder-theme-change", { detail: { themeId: theme.id, mode: theme.mode } }));
}
