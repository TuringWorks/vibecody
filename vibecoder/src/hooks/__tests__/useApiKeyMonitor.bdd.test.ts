 ...
 *  7. vibecoder:api-key-validations event is dispatched on every validation run
 *  8. "vibecoder:providers-updated" event triggers a re-validation
...
// ── Scenario 7: vibecoder:api-key-validations event is dispatched ───────────────

describe('Given a validation run completes', () => {
  it('When the run fires, Then "vibecoder:api-key-validations" custom event is dispatched', async () => {
    mockInvoke.mockResolvedValue([validResult('openai')]);
    const events: CustomEvent[] = [];
    window.addEventListener('vibecoder:api-key-validations', (e) => events.push(e as CustomEvent));
...
// ── Scenario 8: vibecoder:providers-updated triggers re-validation ──────────────

describe('Given the user saves new API keys (vibecoder:providers-updated fires)', () => {
  it('When the event fires, Then a re-validation is triggered', async () => {
    ...
    act(() => { window.dispatchEvent(new CustomEvent('vibecoder:providers-updated')); });
