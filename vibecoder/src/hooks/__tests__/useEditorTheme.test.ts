...
describe('useEditorTheme', () => {
  ...
  it('returns a themeName string starting with "vibecoder-"', () => {
    const { result } = renderHook(() => useEditorTheme());
    expect(result.current.themeName).toMatch(/^vibecoder-/);
  });
  ...
  it('updates themeName when a "vibecoder-theme-change" event fires', async () => {
    const { result } = renderHook(() => useEditorTheme());

    act(() => {
      window.dispatchEvent(
        new CustomEvent('vibecoder-theme-change', { detail: { themeId: 'light-default' } }),
      );
    });
  ...
  it('updates themeName when a storage event fires with vibecoder-theme-id key', async () => {
    const { result } = renderHook(() => useEditorTheme());

    act(() => {
      window.dispatchEvent(
        new StorageEvent('storage', { key: 'vibecoder-theme-id', newValue: 'light-default' }),
      );
    });
  ...
  it('removes event listeners on unmount', () => {
    ...
    // Verify at least one vibecoder-theme-change listener was removed
    const removed = removeSpy.mock.calls.some(([type]) => type === 'vibecoder-theme-change');
  ...
  it('defineTheme updates themeName from the monaco call', () => {
    ...
    // After defineTheme the name should still be vibecoder-<something>
    expect(result.current.themeName).toMatch(/^vibecoder-/);
  });

  it('handles vibecoder-theme-change with no detail gracefully', () => {
    const { result } = renderHook(() => useEditorTheme());
    const originalTheme = result.current.themeName;

    act(() => {
      window.dispatchEvent(new CustomEvent('vibecoder-theme-change', { detail: null }));
    });
