 ...
 * Emits a custom event "vibecoder:api-key-validations" so SettingsPanel can display inline statuses
...
export function useApiKeyMonitor({ toast, addNotification, osNotifications }: UseApiKeyMonitorOpts) {
  ...
  const runValidation = useCallback(async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent("vibecoder:api-key-validations", { detail: map }));
  ...
  useEffect(() => {
    ...
    window.addEventListener("vibecoder:providers-updated", onKeysChanged);
    return () => window.removeEventListener("vibecoder:providers-updated", onKeysChanged);
  }, [runValidation]);
