/**
 * useEditorTheme — Syncs Monaco editor themes with VibeCoder's CSS variable theme system.
 *
 * Generates a Monaco theme from the active VibeCoder theme's CSS variables, registers it,
...
/** Build a full Monaco IStandaloneThemeData from a VibeCoder ThemeDef */
...
export function useEditorTheme() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("vibecoder-theme-change", handler);

    // Also listen for storage events (cross-tab sync)
    const storageHandler = (e: StorageEvent) => {
      if (e.key === "vibecoder-theme-id" && e.newValue) {
    ...
    return () => {
      window.removeEventListener("vibecoder-theme-change", handler);
...
/** Utility: build a Monaco theme from a VibeCoder theme ID (for external use) */
