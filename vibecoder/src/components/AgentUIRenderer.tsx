...
export interface VibeCoderBlock {
    type: "buttons" | "form" | "table";
    props: Record<string, unknown>;
    rawText: string;
}

export interface VibeCoderAction {
...
const BLOCK_RE = /:::vibe-coder:(buttons|form|table)\s*\n([\s\S]*?)\n:::/g;

/**
 * Extract structured vibe-coder blocks from agent text output.
 * Each block is fenced with `:::vibe-coder:<type>` / `:::` and contains a JSON body.
 */
export function parseVibeCoderBlocks(text: string): VibeCoderBlock[] {
    const blocks: VibeCoderBlock[] = [];
    let match: RegExpExecArray | null;
    // Reset lastIndex in case the regex was used before
    BLOCK_RE.lastIndex = 0;
    while ((match = BLOCK_RE.exec(text)) !== null) {
        const blockType = match[1] as VibeCoderBlock["type"];
...
/**
 * Return the input text with all vibe-coder blocks stripped out.
 */
export function stripVibeCoderBlocks(text: string): string {
    BLOCK_RE.lastIndex = 0;
...
const S = {
    ...
    label: { display: "block", fontSize: "var(--font-size-sm)", color: "var(--text-secondary)", marginBottom: 2 },
    input: {
        width: "100%",
...
function ButtonsBlock({ props, onAction }: { props: Record<string, unknown>; onAction: (a: VibeCoderAction) => void }) {
...
function FormBlock({ props, onAction }: { props: Record<string, unknown>; onAction: (a: VibeCoderAction) => void }) {
...
interface AgentUIRendererProps {
    blocks: VibeCoderBlock[];
    onAction: (action: VibeCoderAction) => void;
}
