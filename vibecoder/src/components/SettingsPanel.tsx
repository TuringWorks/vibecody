...
// `vibecoder-jobs` to keep storage migrations single-key.
...
const STORAGE_KEYS = {
  profile: "vibecoder-profile",
  theme: "vibecoder-theme-id",
  themeMode: "vibecoder-theme",
  fontSize: "vibecoder-font-size",
  density: "vibecoder-density",
  oauth: "vibecoder-oauth",
  customizations: "vibecoder-customizations",
  sessions: "vibecoder-sessions",
  jobs: "vibecoder-jobs",
};
...
function AppearanceSection() {
  ...
  const applyDensity = (d: typeof density) => {
    setDensity(d);
    localStorage.setItem("vibecoder-density", d);
...
function CustomizationsSection() {
  ...
  const loadCustom = (c: SavedCustomization) => {
    ...
    localStorage.setItem("vibecoder-density", c.density);
  ...
  const exportAll = () => {
    ...
    a.href = url; a.download = "vibecoder-settings.json"; a.click();
  ...
  const resetAll = () => {
    ...
    localStorage.removeItem("vibecoder-density");
...
function ApiKeysSection() {
  ...
  useEffect(() => {
    ...
    autoSaveTimerRef.current = setTimeout(async () => {
      try {
        ...
        window.dispatchEvent(new CustomEvent("vibecoder:providers-updated", { detail: providers }));
  ...
  useEffect(() => {
    ...
    window.addEventListener("vibecoder:api-key-validations", onValidations);
    ...
    return () => window.removeEventListener("vibecoder:api-key-validations", onValidations);
  }, []);
...
// persist to a single `vibecoder-sessions` JSON blob in localStorage so a
    ...
    <div>
      ...
      <div style={rowStyle}>
        <div>
          ...
          <div style={hintStyle}>Open the most recent session automatically when VibeCoder launches.</div>
        </div>
        <input
          type="checkbox"
