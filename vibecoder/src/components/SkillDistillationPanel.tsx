...
const metricBox: React.CSSProperties = { textAlign: "center", padding: 12, borderRadius: "var(--radius-sm)", background: "var(--bg-tertiary)", flex: 1 };
const badgeStyle = (color: string): React.CSSProperties => ({ fontSize: "var(--font-size-xs)", padding: "2px 6px", borderRadius: 3, background: color, color: "var(--btn-primary-fg, #fff)", marginLeft: 6 });
    ...
    <div className="panel-container">
      ...
      {tab === "export" && (
        <div className="panel-card">
          ...
          {exported ? (
            <pre style={{ whiteSpace: "pre-wrap", fontSize: "var(--font-size-sm)", fontFamily: "monospace", margin: 0, maxHeight: 400, overflow: "auto" }}>{exported}</pre>
          ) : (
