...
function extractLabel(message: string): string {
  // Strip "vibecoder-checkpoint: " prefix if present
  const prefix = "vibecoder-checkpoint: ";
  if (message.startsWith(prefix)) return message.slice(prefix.length);
  // Strip "vibecoder-pre-ai: " prefix from stash entries
  const preAi = "vibecoder-pre-ai: ";
