...
describe('SettingsPanel', () => {
  ...
  describe('Sessions section', () => {
    ...
    it('toggling "Recap on tab close" persists to localStorage', () => {
      ...
      const stored = JSON.parse(localStorage.getItem('vibecoder-sessions') || '{}');
    ...
    it('hydrates initial state from localStorage', () => {
      localStorage.setItem('vibecoder-sessions', JSON.stringify({
  ...
  describe('Background Jobs section (J1.5)', () => {
    ...
    it('toggling auto-recap persists under vibecoder-jobs', () => {
      render(<SettingsPanel />);
      fireEvent.click(screen.getByText('Background Jobs'));
      const cb = screen.getByLabelText('Auto-recap on terminal state') as HTMLInputElement;
      fireEvent.click(cb);
      const stored = JSON.parse(localStorage.getItem('vibecoder-jobs') || '{}');
      expect(stored.autoRecapOnTerminal).toBe(false);
    });

    it('selecting LLM job generator persists under vibecoder-jobs', () => {
      render(<SettingsPanel />);
      fireEvent.click(screen.getByText('Background Jobs'));
    ...
    it('hydrates initial state from localStorage', () => {
      localStorage.setItem('vibecoder-jobs', JSON.stringify({
    ...
    it('falls back to defaults when vibecoder-jobs is corrupt', () => {
      localStorage.setItem('vibecoder-jobs', '{not json');
      render(<SettingsPanel />);
      fireEvent.click(screen.getByText('Background Jobs'));
      expect(screen.getByLabelText('Auto-recap on terminal state')).toBeChecked();
      expect(screen.getByLabelText('Job generator: heuristic')).toHaveAttribute('aria-pressed', 'true');
