 ...
 *  7. vibecoder:daemon-status custom event updates online state
 *  8. Refreshes job list when vibecoder:daemon-status fires with online=true
...
// ── Scenario 7: vibecoder:daemon-status custom event ─────────────────────────────

describe('Given the vibecoder:daemon-status event fires', () => {
  it('When online:true is dispatched, Then the online indicator appears', async () => {
    mockFetchOnline();
    render(<BackgroundJobsPanel daemonUrl="http://localhost:7878" />);

    act(() => {
      window.dispatchEvent(new CustomEvent('vibecoder:daemon-status', {
...
describe('Given a completed job has a recap', () => {
  ...
  it('When Resume is clicked, Then POST /v1/resume is called with from_recap_id', async () => {
    ...
    await waitFor(() => {
      ...
      expect(body.client).toBe('vibecoder-jobs');
    });
  });
});
