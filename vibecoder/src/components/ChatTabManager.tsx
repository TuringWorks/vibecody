...
// Updated from ~/.vibecoder/adventure-names.json once the backend responds.
...
export function ChatTabManager({
    ...
    // Google Docs-style sync: when Watch switches to a session, VibeCoder follows.
    ...
    const isRecapOnCloseEnabled = (): boolean => {
        try {
            const raw = localStorage.getItem("vibecoder-sessions");
            if (!raw) return true;
    ...
    // persisted under the `vibecoder-sessions` JSON blob. Reads the
    ...
    useEffect(() => {
        ...
        try { raw = localStorage.getItem("vibecoder-sessions"); } catch { /* private browsing */ }
        if (!raw) return;
        let auto: boolean;
        try {
    ...
    useEffect(() => {
        ...
        window.addEventListener("vibecoder:inject-context", handler);
        return () => window.removeEventListener("vibecoder:inject-context", handler);
    }, [activeTabId]);
